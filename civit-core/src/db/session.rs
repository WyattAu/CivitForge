...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        .fetch_one(&self.db)
        .await
        .map_err(|e| CoreError::Database(format!("create_session: {e}")))?;
    ...
    pub async fn validate_session(&self, raw_token: &str) -> Result<Session> {
        ...
        let row = sqlx::query_as::<_, Session>("SELECT * FROM sessions WHERE token_hash = $1")
            ...
            .fetch_one(&self.db)
            .await
            .map_err(|e| CoreError::Auth(format!("session not found: {e}")))?;
        ...
        sqlx::query("UPDATE sessions SET last_active_at = NOW() WHERE id = $1")
            ...
            .execute(&self.db)
            .await
            .map_err(|e| CoreError::Database(format!("update last_active_at: {e}")))?;
    ...
    pub async fn extend_session(
        ...
        let current: Option<(Uuid,)> =
            sqlx::query_as("SELECT id FROM sessions WHERE token_hash = $1")
                ...
                .await
                .map_err(|e| CoreError::Database(format!("lookup session: {e}")))?;

        let session_id = current
            .ok_or_else(|| CoreError::Auth("session not found".into()))?
            .0;
        ...
        sqlx::query("UPDATE sessions SET expires_at = $1, last_active_at = NOW() WHERE id = $2")
            ...
            .execute(&self.db)
            .await
            .map_err(|e| CoreError::Database(format!("extend_session: {e}")))?;

        let row = sqlx::query_as::<_, Session>("SELECT * FROM sessions WHERE id = $1")
            .bind(session_id)
            .fetch_one(&self.db)
            .await
            .map_err(|e| CoreError::Database(format!("fetch extended session: {e}")))?;
    ...
    pub async fn revoke_session(&self, raw_token: &str) -> Result<()> {
        ...
        sqlx::query("DELETE FROM sessions WHERE token_hash = $1")
            ...
            .execute(&self.db)
            .await
            .map_err(|e| CoreError::Database(format!("revoke_session: {e}")))?;
        Ok(())
    }
    ...
    pub async fn revoke_all_sessions(&self, user_id: Uuid) -> Result<u64> {
        let result = sqlx::query("DELETE FROM sessions WHERE user_id = $1")
            ...
            .execute(&self.db)
            .await
            .map_err(|e| CoreError::Database(format!("revoke_all_sessions: {e}")))?;
    ...
    pub async fn cleanup_expired(&self) -> Result<u64> {
        let result = sqlx::query("DELETE FROM sessions WHERE expires_at < NOW()")
            .execute(&self.db)
            .await
            .map_err(|e| CoreError::Database(format!("cleanup_expired: {e}")))?;
        Ok(result.rows_affected())
    }
}
