...
impl Default for GeoLocationV16 {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct AuditTrailEventV16 {
    ...
    pub geo_location: Option<GeoLocationV16>,
    pub risk_score: u32,
    pub compliance_status: ComplianceStatusV16,
    pub created_at: DateTime<Utc>,
}

impl AuditTrailEventV16 {
    pub fn new(
        event_type: &str,
        resource_type: &str,
        resource_id: &str,
        ...
        Self {
            ...
            geo_location: None,
            risk_score: 0,
            compliance_status: ComplianceStatusV16::Unknown,
            created_at: Utc::now(),
        }
    }
    ...
    pub fn with_compliance_status(mut self, status: ComplianceStatusV16) -> Self {
    ...
    pub fn is_compliant(&self) -> bool {
        self.compliance_status == ComplianceStatusV16::Compliant
    }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditTrailQueryV16 {
    ...
    pub request_id: Option<String>,
    pub session_id: Option<String>,
    pub compliance_status: Option<ComplianceStatusV16>,
...
impl Default for AuditTrailQueryV16 {
    fn default() -> Self {
        Self {
            event_type: None,
            resource_type: None,
...
impl AuditTrailQueryV16 {
    pub fn matches(&self, event: &AuditTrailEventV16) -> bool {
...
pub struct AuditTrailRecorderV16 {
    events: std::sync::Mutex<Vec<AuditTrailEventV16>>,
}

impl AuditTrailRecorderV16 {
    pub fn new() -> Self {
        Self {
    ...
    pub fn record(&self, event: AuditTrailEventV16) {
        let mut events = self.events.lock().unwrap();
        events.push(event);
    }

    pub fn search(&self, query: &AuditTrailQueryV16) -> Vec<AuditTrailEventV16> {
    ...
    pub fn get_session_events(&self, session_id: &str) -> Vec<AuditTrailEventV16> {
        let events = self.events.lock().unwrap();
        events
            .iter()
            .filter(|e| e.session_id.as_deref() == Some(session_id))
    ...
    pub fn get_request_events(&self, request_id: &str) -> Vec<AuditTrailEventV16> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn get_events_by_geo_country(&self, country: &str) -> Vec<AuditTrailEventV16> {
        let events = self.events.lock().unwrap();
        events
            .iter()
            .filter(|e| {
    ...
    pub fn get_events_by_compliance_status(&self, status: ComplianceStatusV16) -> Vec<AuditTrailEventV16> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn get_high_risk_events(&self, threshold: u32) -> Vec<AuditTrailEventV16> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn compliance_audit(
        ...
        let compliant_count = relevant
            .iter()
            .filter(|e| e.compliance_status == ComplianceStatusV16::Compliant)
            .count() as u32;
    ...
    pub fn detect_anomalies(&self) -> Vec<AnomalyDetectionResultV16> {
        let events = self.events.lock().unwrap();
        ...
        for (actor, count) in &actor_event_counts {
            if *count as f64 > avg_events_per_actor * 2.0 && *count > 10 {
                anomalies.push(AnomalyDetectionResultV16 {
                    anomaly_type: AnomalyTypeV16::HighActivity,
        ...
        for event in events.iter() {
            if event.compliance_status == ComplianceStatusV16::NonCompliant {
                failed_count += 1;
            }
            total_count += 1;
        }
        ...
        if total_count > 10 && (failed_count as f64 / total_count as f64) > 0.3 {
            anomalies.push(AnomalyDetectionResultV16 {
                anomaly_type: AnomalyTypeV16::ComplianceDrift,
                actor_id: None,
                description: format!(
        ...
        for (actor, ips) in &actor_ips {
            if ips.len() > 3 {
                anomalies.push(AnomalyDetectionResultV16 {
                    anomaly_type: AnomalyTypeV16::UnusualAccess,
                    actor_id: Some(actor.clone()),
                    description: format!(
    ...
    pub fn risk_assessment(&self) -> RiskAssessmentResultV16 {
        let events = self.events.lock().unwrap();
    ...
    pub fn export(&self, query: &AuditTrailQueryV16) -> AuditExportV16 {
        let events = self.search(query);
        AuditExportV16 {
            format: ExportFormatV16::Json,
...
impl Default for AuditTrailRecorderV16 {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceAuditResultV16 {
    pub event_type: String,
...
pub struct ForensicsExportV16 {
    ...
    pub events: Vec<AuditTrailEventV16>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditExportV16 {
    pub format: ExportFormatV16,
...
#[serde(rename_all = "snake_case")]
pub enum ExportFormatV16 {
    Json,
    Csv,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditTrailBuilderV16 {
    event_type: String,
    ...
    geo_location: Option<GeoLocationV16>,
    risk_score: u32,
    compliance_status: ComplianceStatusV16,
}

impl AuditTrailBuilderV16 {
    ...
    pub fn geo_location(mut self, geo: GeoLocationV16) -> Self {
    ...
    pub fn compliance_status(mut self, status: ComplianceStatusV16) -> Self {
        self.compliance_status = status;
        self
    }

    pub fn build(self) -> AuditTrailEventV16 {
        AuditTrailEventV16 {
            id: uuid::Uuid::new_v4().to_string(),
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AnomalyDetectionResultV16 {
    pub anomaly_type: AnomalyTypeV16,
    pub actor_id: Option<String>,
    pub description: String,
    pub risk_level: RiskLevelV16,
    pub detected_at: DateTime<Utc>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RiskAssessmentResultV16 {
    pub total_events: u32,
...
pub fn record_compliance_event_v16(
    recorder: &AuditTrailRecorderV16,
    repo_id: &str,
    action: &str,
    framework_name: &str,
    details: serde_json::Value,
    ...
    let event = AuditTrailBuilderV16::new("compliance", "repository", repo_id, action)
...
pub fn record_security_event_v16(
    recorder: &AuditTrailRecorderV16,
    repo_id: &str,
    action: &str,
    scan_type: &str,
    ...
    let event = AuditTrailBuilderV16::new("security", "repository", repo_id, action)
        .details(serde_json::json!({
            "scan_type": scan_type,
            "details": details,
        }))
...
mod tests {
    ...
    fn make_event(event_type: &str, resource_type: &str, resource_id: &str, action: &str) -> AuditTrailEventV16 {
        AuditTrailEventV16 {
            id: uuid::Uuid::new_v4().to_string(),
            event_type: event_type.into(),
            resource_type: resource_type.into(),
    ...
    #[test]
    fn test_geo_location_v16_new() {
        let geo = GeoLocationV16::new();
    ...
    #[test]
    fn test_geo_location_v16_with_fields() {
        let geo = GeoLocationV16::new()
            .with_country("US")
            .with_region("CA")
    ...
    #[test]
    fn test_audit_trail_event_v16_new() {
        let event = AuditTrailEventV16::new("access", "repo", "r1", "read");
        ...
        assert_eq!(event.compliance_status, ComplianceStatusV16::Unknown);
    }

    #[test]
    fn test_audit_trail_event_v16_with_risk_score() {
        let event = AuditTrailEventV16::new("access", "repo", "r1", "read")
            .with_risk_score(75)
            .with_compliance_status(ComplianceStatusV16::NonCompliant);
        assert_eq!(event.risk_score, 75);
        assert_eq!(event.compliance_status, ComplianceStatusV16::NonCompliant);
    }
    ...
    fn test_event_helpers() {
        let event = AuditTrailEventV16::new("access", "repo", "r1", "read")
            .with_risk_score(80)
            .with_compliance_status(ComplianceStatusV16::Compliant);
    ...
    fn test_compliance_status_risk_contribution() {
        assert_eq!(ComplianceStatusV16::Compliant.risk_contribution(), 0);
        assert_eq!(ComplianceStatusV16::Partial.risk_contribution(), 50);
        assert_eq!(ComplianceStatusV16::NonCompliant.risk_contribution(), 100);
        assert_eq!(ComplianceStatusV16::Unknown.risk_contribution(), 25);
    }

    #[test]
    fn test_compliance_status_display_name() {
        assert_eq!(ComplianceStatusV16::Compliant.display_name(), "Compliant");
        assert_eq!(ComplianceStatusV16::NonCompliant.display_name(), "Non-Compliant");
    }

    #[test]
    fn test_audit_trail_query_v16_matches() {
        let mut event = AuditTrailEventV16::new("access", "repo", "r1", "read");
        event.request_id = Some("req-1".into());
        event.session_id = Some("sess-1".into());
        event.risk_score = 50;
        event.compliance_status = ComplianceStatusV16::Compliant;
        let query = AuditTrailQueryV16 {
            request_id: Some("req-1".into()),
            ..AuditTrailQueryV16::default()
        };
        assert!(query.matches(&event));
        let query2 = AuditTrailQueryV16 {
    ...
    #[test]
    fn test_audit_trail_query_v16_matches_risk_score() {
        let mut event = AuditTrailEventV16::new("access", "repo", "r1", "read");
        event.risk_score = 75;
        let query = AuditTrailQueryV16 {
    ...
    #[test]
    fn test_audit_trail_query_v16_matches_compliance_status() {
        let mut event = AuditTrailEventV16::new("access", "repo", "r1", "read");
        event.compliance_status = ComplianceStatusV16::Compliant;
        let query = AuditTrailQueryV16 {
            compliance_status: Some(ComplianceStatusV16::Compliant),
            ..AuditTrailQueryV16::default()
        };
        assert!(query.matches(&event));
        let query2 = AuditTrailQueryV16 {
            compliance_status: Some(ComplianceStatusV16::NonCompliant),
            ..AuditTrailQueryV16::default()
        };
        assert!(!query2.matches(&event));
    }
    ...
    #[test]
    fn test_audit_trail_recorder_v16_record_and_search() {
        let recorder = AuditTrailRecorderV16::new();
        let event = AuditTrailEventV16::new("access", "repo", "r1", "read");
        recorder.record(event);
        assert_eq!(recorder.count(), 1);
        let results = recorder.search(&AuditTrailQueryV16::default());
        assert_eq!(results.len(), 1);
    }

    #[test]
    fn test_audit_trail_recorder_v16_session_tracking() {
        let recorder = AuditTrailRecorderV16::new();
        let mut e1 = AuditTrailEventV16::new("access", "repo", "r1", "read");
        e1.session_id = Some("sess-1".into());
        let mut e2 = AuditTrailEventV16::new("access", "repo", "r1", "read");
        e2.session_id = Some("sess-1".into());
        let mut e3 = AuditTrailEventV16::new("access", "repo", "r1", "read");
    ...
    #[test]
    fn test_audit_trail_recorder_v16_geo_tracking() {
        let recorder = AuditTrailRecorderV16::new();
        let mut e1 = AuditTrailEventV16::new("access", "repo", "r1", "read");
        e1.geo_location = Some(GeoLocationV16::new().with_country("US"));
        let mut e2 = AuditTrailEventV16::new("access", "repo", "r2", "read");
        e2.geo_location = Some(GeoLocationV16::new().with_country("UK"));
        let mut e3 = AuditTrailEventV16::new("access", "repo", "r3", "read");
        e3.geo_location = Some(GeoLocationV16::new().with_country("US"));
        recorder.record(e1);
        recorder.record(e2);
        recorder.record(e3);
    ...
    #[test]
    fn test_audit_trail_recorder_v16_compliance_tracking() {
        let recorder = AuditTrailRecorderV16::new();
        let mut e1 = AuditTrailEventV16::new("access", "repo", "r1", "read");
        e1.compliance_status = ComplianceStatusV16::Compliant;
        let mut e2 = AuditTrailEventV16::new("access", "repo", "r2", "read");
        e2.compliance_status = ComplianceStatusV16::NonCompliant;
        let mut e3 = AuditTrailEventV16::new("access", "repo", "r3", "read");
        e3.compliance_status = ComplianceStatusV16::Compliant;
        recorder.record(e1);
        recorder.record(e2);
        recorder.record(e3);
        assert_eq!(recorder.get_events_by_compliance_status(ComplianceStatusV16::Compliant).len(), 2);
        assert_eq!(recorder.get_events_by_compliance_status(ComplianceStatusV16::NonCompliant).len(), 1);
    }

    #[test]
    fn test_audit_trail_recorder_v16_high_risk_events() {
        let recorder = AuditTrailRecorderV16::new();
        let mut e1 = AuditTrailEventV16::new("access", "repo", "r1", "read");
        e1.risk_score = 90;
        let mut e2 = AuditTrailEventV16::new("access", "repo", "r2", "read");
    ...
    #[test]
    fn test_audit_trail_recorder_v16_compliance_audit() {
        let recorder = AuditTrailRecorderV16::new();
        let mut e1 = AuditTrailEventV16::new("security", "repo", "r1", "scan");
        e1.actor_id = Some("user-1".into());
        e1.risk_score = 75;
        e1.compliance_status = ComplianceStatusV16::Compliant;
        let mut e2 = AuditTrailEventV16::new("security", "repo", "r2", "scan");
        e2.actor_id = Some("user-1".into());
        e2.risk_score = 25;
        e2.compliance_status = ComplianceStatusV16::NonCompliant;
        let mut e3 = AuditTrailEventV16::new("security", "repo", "r1", "fix");
    ...
    #[test]
    fn test_audit_trail_recorder_v16_forensics_export() {
        let recorder = AuditTrailRecorderV16::new();
        let mut e1 = AuditTrailEventV16::new("access", "repo", "r1", "read");
        e1.actor_id = Some("user-1".into());
        e1.session_id = Some("sess-1".into());
        ...
        e1.risk_score = 50;
        e1.compliance_status = ComplianceStatusV16::Compliant;
        let mut e2 = AuditTrailEventV16::new("access", "repo", "r2", "write");
        e2.actor_id = Some("user-1".into());
        e2.session_id = Some("sess-1".into());
    ...
    #[test]
    fn test_audit_trail_recorder_v16_detect_anomalies() {
        let recorder = AuditTrailRecorderV16::new();
        for i in 0..20 {
            let mut e = AuditTrailEventV16::new("access", "repo", &format!("r{}", i), "read");
            e.actor_id = Some("user-1".into());
            e.risk_score = 10;
            e.compliance_status = ComplianceStatusV16::Compliant;
            recorder.record(e);
        }
        for i in 0..5 {
            let mut e = AuditTrailEventV16::new("access", "repo", &format!("r{}", i + 20), "read");
        ...
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV16::HighActivity));
    }

    #[test]
    fn test_audit_trail_recorder_v16_detect_anomalies_compliance_drift() {
        let recorder = AuditTrailRecorderV16::new();
        for i in 0..15 {
            let mut e = AuditTrailEventV16::new("security", "repo", &format!("r{}", i), "scan");
            e.actor_id = Some("user-1".into());
            e.compliance_status = ComplianceStatusV16::NonCompliant;
            recorder.record(e);
        }
        let anomalies = recorder.detect_anomalies();
        assert!(!anomalies.is_empty());
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV16::ComplianceDrift));
    }

    #[test]
    fn test_audit_trail_recorder_v16_detect_anomalies_unusual_access() {
        let recorder = AuditTrailRecorderV16::new();
        let mut e1 = AuditTrailEventV16::new("access", "repo", "r1", "read");
        e1.actor_id = Some("user-1".into());
        e1.ip_address = Some("10.0.0.1".into());
        recorder.record(e1);
        for i in 0..5 {
            let mut e = AuditTrailEventV16::new("access", "repo", &format!("r{}", i + 2), "read");
            e.actor_id = Some("user-1".into());
            e.ip_address = Some(format!("10.0.0.{}", i + 2));
            recorder.record(e);
        }
        let anomalies = recorder.detect_anomalies();
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV16::UnusualAccess));
    }
    ...
    #[test]
    fn test_audit_trail_builder_v16() {
        let geo = GeoLocationV16::new()
            .with_country("US")
            .with_city("New York");
        let event = AuditTrailBuilderV16::new("security", "repo", "r1", "scan")
            .actor_id("user-1")
            .ip_address("10.0.0.1")
            ...
            .geo_location(geo)
            .risk_score(75)
            .compliance_status(ComplianceStatusV16::Compliant)
        ...
        assert_eq!(event.risk_score, 75);
        assert_eq!(event.compliance_status, ComplianceStatusV16::Compliant);
        assert_eq!(event.details["key"], "value");
    }

    #[test]
    fn test_audit_trail_export_v16() {
        let recorder = AuditTrailRecorderV16::new();
        recorder.record(AuditTrailEventV16::new("access", "repo", "r1", "read"));
        let export = recorder.export(&AuditTrailQueryV16::default());
        assert_eq!(export.total_events, 1);
        assert_eq!(export.format, ExportFormatV16::Json);
    }

    #[test]
    fn test_record_compliance_event_v16() {
        let recorder = AuditTrailRecorderV16::new();
        record_compliance_event_v16(
            &recorder,
            "repo-1",
            "assessment_complete",
            "SOC 2",
        ...
        assert_eq!(results[0].compliance_status, ComplianceStatusV16::Compliant);
    }

    #[test]
    fn test_record_security_event_v16() {
        let recorder = AuditTrailRecorderV16::new();
        record_security_event_v16(
            &recorder,
            "repo-1",
            "scan_completed",
            "sast",
    ...
    fn test_risk_level_display_name() {
        assert_eq!(RiskLevelV16::Critical.display_name(), "Critical");
        assert_eq!(RiskLevelV16::Low.display_name(), "Low");
    }

    #[test]
    fn test_export_format_v16_serialization() {
        assert_eq!(
            serde_json::to_string(&ExportFormatV16::Json).unwrap(),
            "\"json\""
        );
        assert_eq!(
            serde_json::to_string(&ExportFormatV16::Csv).unwrap(),
            "\"csv\""
        );
    }

    #[test]
    fn test_anomaly_type_v16_serialization() {
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV16::HighActivity).unwrap(),
            "\"high_activity\""
        );
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV16::ComplianceDrift).unwrap(),
            "\"compliance_drift\""
        );
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV16::GeographicAnomaly).unwrap(),
            "\"geographic_anomaly\""
        );
    }

    #[test]
    fn test_risk_level_v16_serialization() {
        assert_eq!(
            serde_json::to_string(&RiskLevelV16::Critical).unwrap(),
            "\"critical\""
        );
        assert_eq!(
            serde_json::to_string(&RiskLevelV16::Low).unwrap(),
            "\"low\""
        );
    }
}
