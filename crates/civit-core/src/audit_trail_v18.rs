...
impl Default for GeoLocationV18 {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct AuditTrailEventV18 {
    ...
    pub geo_location: Option<GeoLocationV18>,
    pub risk_score: u32,
    pub compliance_status: ComplianceStatusV18,
    pub created_at: DateTime<Utc>,
}

impl AuditTrailEventV18 {
    pub fn new(
        event_type: &str,
        resource_type: &str,
        resource_id: &str,
        ...
        Self {
            ...
            geo_location: None,
            risk_score: 0,
            compliance_status: ComplianceStatusV18::Unknown,
            created_at: Utc::now(),
        }
    }
    ...
    pub fn with_compliance_status(mut self, status: ComplianceStatusV18) -> Self {
    ...
    pub fn is_compliant(&self) -> bool {
        self.compliance_status == ComplianceStatusV18::Compliant
    }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditTrailQueryV18 {
    ...
    pub request_id: Option<String>,
    pub session_id: Option<String>,
    pub compliance_status: Option<ComplianceStatusV18>,
...
impl Default for AuditTrailQueryV18 {
    fn default() -> Self {
        Self {
            event_type: None,
            resource_type: None,
...
impl AuditTrailQueryV18 {
    pub fn matches(&self, event: &AuditTrailEventV18) -> bool {
...
pub struct AuditTrailRecorderV18 {
    events: std::sync::Mutex<Vec<AuditTrailEventV18>>,
}

impl AuditTrailRecorderV18 {
    pub fn new() -> Self {
        Self {
    ...
    pub fn record(&self, event: AuditTrailEventV18) {
        let mut events = self.events.lock().unwrap();
        events.push(event);
    }

    pub fn search(&self, query: &AuditTrailQueryV18) -> Vec<AuditTrailEventV18> {
    ...
    pub fn get_session_events(&self, session_id: &str) -> Vec<AuditTrailEventV18> {
        let events = self.events.lock().unwrap();
        events
            .iter()
            .filter(|e| e.session_id.as_deref() == Some(session_id))
    ...
    pub fn get_request_events(&self, request_id: &str) -> Vec<AuditTrailEventV18> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn get_events_by_geo_country(&self, country: &str) -> Vec<AuditTrailEventV18> {
        let events = self.events.lock().unwrap();
        events
            .iter()
            .filter(|e| {
    ...
    pub fn get_events_by_compliance_status(&self, status: ComplianceStatusV18) -> Vec<AuditTrailEventV18> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn get_high_risk_events(&self, threshold: u32) -> Vec<AuditTrailEventV18> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn compliance_audit(
        ...
        let compliant_count = relevant
            .iter()
            .filter(|e| e.compliance_status == ComplianceStatusV18::Compliant)
            .count() as u32;
    ...
    pub fn detect_anomalies(&self) -> Vec<AnomalyDetectionResultV18> {
        let events = self.events.lock().unwrap();
        ...
        for (actor, count) in &actor_event_counts {
            if *count as f64 > avg_events_per_actor * 2.0 && *count > 10 {
                anomalies.push(AnomalyDetectionResultV18 {
                    anomaly_type: AnomalyTypeV18::HighActivity,
        ...
        for event in events.iter() {
            if event.compliance_status == ComplianceStatusV18::NonCompliant {
                failed_count += 1;
            }
            total_count += 1;
        }
        ...
        if total_count > 10 && (failed_count as f64 / total_count as f64) > 0.3 {
            anomalies.push(AnomalyDetectionResultV18 {
                anomaly_type: AnomalyTypeV18::ComplianceDrift,
                actor_id: None,
                description: format!(
        ...
        for (actor, ips) in &actor_ips {
            if ips.len() > 3 {
                anomalies.push(AnomalyDetectionResultV18 {
                    anomaly_type: AnomalyTypeV18::UnusualAccess,
                    actor_id: Some(actor.clone()),
                    description: format!(
    ...
    pub fn risk_assessment(&self) -> RiskAssessmentResultV18 {
        let events = self.events.lock().unwrap();
    ...
    pub fn export(&self, query: &AuditTrailQueryV18) -> AuditExportV18 {
        let events = self.search(query);
        AuditExportV18 {
            format: ExportFormatV18::Json,
...
impl Default for AuditTrailRecorderV18 {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceAuditResultV18 {
    pub event_type: String,
...
pub struct ForensicsExportV18 {
    ...
    pub events: Vec<AuditTrailEventV18>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditExportV18 {
    pub format: ExportFormatV18,
...
#[serde(rename_all = "snake_case")]
pub enum ExportFormatV18 {
    Json,
    Csv,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditTrailBuilderV18 {
    event_type: String,
    ...
    geo_location: Option<GeoLocationV18>,
    risk_score: u32,
    compliance_status: ComplianceStatusV18,
}

impl AuditTrailBuilderV18 {
    ...
    pub fn geo_location(mut self, geo: GeoLocationV18) -> Self {
    ...
    pub fn compliance_status(mut self, status: ComplianceStatusV18) -> Self {
        self.compliance_status = status;
        self
    }

    pub fn build(self) -> AuditTrailEventV18 {
        AuditTrailEventV18 {
            id: uuid::Uuid::new_v4().to_string(),
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AnomalyDetectionResultV18 {
    pub anomaly_type: AnomalyTypeV18,
    pub actor_id: Option<String>,
    pub description: String,
    pub risk_level: RiskLevelV18,
    pub detected_at: DateTime<Utc>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RiskAssessmentResultV18 {
    pub total_events: u32,
...
pub fn record_compliance_event_v18(
    recorder: &AuditTrailRecorderV18,
    repo_id: &str,
    action: &str,
    framework_name: &str,
    details: serde_json::Value,
    ...
    let event = AuditTrailBuilderV18::new("compliance", "repository", repo_id, action)
...
pub fn record_security_event_v18(
    recorder: &AuditTrailRecorderV18,
    repo_id: &str,
    action: &str,
    scan_type: &str,
    ...
    let event = AuditTrailBuilderV18::new("security", "repository", repo_id, action)
        .details(serde_json::json!({
            "scan_type": scan_type,
            "details": details,
        }))
...
mod tests {
    ...
    fn make_event(event_type: &str, resource_type: &str, resource_id: &str, action: &str) -> AuditTrailEventV18 {
        AuditTrailEventV18 {
            id: uuid::Uuid::new_v4().to_string(),
            event_type: event_type.into(),
            resource_type: resource_type.into(),
    ...
    #[test]
    fn test_geo_location_v18_new() {
        let geo = GeoLocationV18::new();
    ...
    #[test]
    fn test_geo_location_v18_with_fields() {
        let geo = GeoLocationV18::new()
            .with_country("US")
            .with_region("CA")
    ...
    #[test]
    fn test_audit_trail_event_v18_new() {
        let event = AuditTrailEventV18::new("access", "repo", "r1", "read");
        ...
        assert_eq!(event.compliance_status, ComplianceStatusV18::Unknown);
    }

    #[test]
    fn test_audit_trail_event_v18_with_risk_score() {
        let event = AuditTrailEventV18::new("access", "repo", "r1", "read")
            .with_risk_score(75)
            .with_compliance_status(ComplianceStatusV18::NonCompliant);
        assert_eq!(event.risk_score, 75);
        assert_eq!(event.compliance_status, ComplianceStatusV18::NonCompliant);
    }
    ...
    fn test_event_helpers() {
        let event = AuditTrailEventV18::new("access", "repo", "r1", "read")
            .with_risk_score(80)
            .with_compliance_status(ComplianceStatusV18::Compliant);
    ...
    fn test_compliance_status_risk_contribution() {
        assert_eq!(ComplianceStatusV18::Compliant.risk_contribution(), 0);
        assert_eq!(ComplianceStatusV18::Partial.risk_contribution(), 50);
        assert_eq!(ComplianceStatusV18::NonCompliant.risk_contribution(), 100);
        assert_eq!(ComplianceStatusV18::Unknown.risk_contribution(), 25);
    }

    #[test]
    fn test_compliance_status_display_name() {
        assert_eq!(ComplianceStatusV18::Compliant.display_name(), "Compliant");
        assert_eq!(ComplianceStatusV18::NonCompliant.display_name(), "Non-Compliant");
    }

    #[test]
    fn test_audit_trail_query_v18_matches() {
        let mut event = AuditTrailEventV18::new("access", "repo", "r1", "read");
        event.request_id = Some("req-1".into());
        event.session_id = Some("sess-1".into());
        event.risk_score = 50;
        event.compliance_status = ComplianceStatusV18::Compliant;
        let query = AuditTrailQueryV18 {
            request_id: Some("req-1".into()),
            ..AuditTrailQueryV18::default()
        };
        assert!(query.matches(&event));
        let query2 = AuditTrailQueryV18 {
    ...
    #[test]
    fn test_audit_trail_query_v18_matches_risk_score() {
        let mut event = AuditTrailEventV18::new("access", "repo", "r1", "read");
        event.risk_score = 75;
        let query = AuditTrailQueryV18 {
    ...
    #[test]
    fn test_audit_trail_query_v18_matches_compliance_status() {
        let mut event = AuditTrailEventV18::new("access", "repo", "r1", "read");
        event.compliance_status = ComplianceStatusV18::Compliant;
        let query = AuditTrailQueryV18 {
            compliance_status: Some(ComplianceStatusV18::Compliant),
            ..AuditTrailQueryV18::default()
        };
        assert!(query.matches(&event));
        let query2 = AuditTrailQueryV18 {
            compliance_status: Some(ComplianceStatusV18::NonCompliant),
            ..AuditTrailQueryV18::default()
        };
        assert!(!query2.matches(&event));
    }
    ...
    #[test]
    fn test_audit_trail_recorder_v18_record_and_search() {
        let recorder = AuditTrailRecorderV18::new();
        let event = AuditTrailEventV18::new("access", "repo", "r1", "read");
        recorder.record(event);
        assert_eq!(recorder.count(), 1);
        let results = recorder.search(&AuditTrailQueryV18::default());
        assert_eq!(results.len(), 1);
    }

    #[test]
    fn test_audit_trail_recorder_v18_session_tracking() {
        let recorder = AuditTrailRecorderV18::new();
        let mut e1 = AuditTrailEventV18::new("access", "repo", "r1", "read");
        e1.session_id = Some("sess-1".into());
        let mut e2 = AuditTrailEventV18::new("access", "repo", "r1", "read");
        e2.session_id = Some("sess-1".into());
        let mut e3 = AuditTrailEventV18::new("access", "repo", "r1", "read");
    ...
    #[test]
    fn test_audit_trail_recorder_v18_geo_tracking() {
        let recorder = AuditTrailRecorderV18::new();
        let mut e1 = AuditTrailEventV18::new("access", "repo", "r1", "read");
        e1.geo_location = Some(GeoLocationV18::new().with_country("US"));
        let mut e2 = AuditTrailEventV18::new("access", "repo", "r2", "read");
        e2.geo_location = Some(GeoLocationV18::new().with_country("UK"));
        let mut e3 = AuditTrailEventV18::new("access", "repo", "r3", "read");
        e3.geo_location = Some(GeoLocationV18::new().with_country("US"));
        recorder.record(e1);
        recorder.record(e2);
        recorder.record(e3);
    ...
    #[test]
    fn test_audit_trail_recorder_v18_compliance_tracking() {
        let recorder = AuditTrailRecorderV18::new();
        let mut e1 = AuditTrailEventV18::new("access", "repo", "r1", "read");
        e1.compliance_status = ComplianceStatusV18::Compliant;
        let mut e2 = AuditTrailEventV18::new("access", "repo", "r2", "read");
        e2.compliance_status = ComplianceStatusV18::NonCompliant;
        let mut e3 = AuditTrailEventV18::new("access", "repo", "r3", "read");
        e3.compliance_status = ComplianceStatusV18::Compliant;
        recorder.record(e1);
        recorder.record(e2);
        recorder.record(e3);
        assert_eq!(recorder.get_events_by_compliance_status(ComplianceStatusV18::Compliant).len(), 2);
        assert_eq!(recorder.get_events_by_compliance_status(ComplianceStatusV18::NonCompliant).len(), 1);
    }

    #[test]
    fn test_audit_trail_recorder_v18_high_risk_events() {
        let recorder = AuditTrailRecorderV18::new();
        let mut e1 = AuditTrailEventV18::new("access", "repo", "r1", "read");
        e1.risk_score = 90;
        let mut e2 = AuditTrailEventV18::new("access", "repo", "r2", "read");
    ...
    #[test]
    fn test_audit_trail_recorder_v18_compliance_audit() {
        let recorder = AuditTrailRecorderV18::new();
        let mut e1 = AuditTrailEventV18::new("security", "repo", "r1", "scan");
        e1.actor_id = Some("user-1".into());
        e1.risk_score = 75;
        e1.compliance_status = ComplianceStatusV18::Compliant;
        let mut e2 = AuditTrailEventV18::new("security", "repo", "r2", "scan");
        e2.actor_id = Some("user-1".into());
        e2.risk_score = 25;
        e2.compliance_status = ComplianceStatusV18::NonCompliant;
        let mut e3 = AuditTrailEventV18::new("security", "repo", "r1", "fix");
    ...
    #[test]
    fn test_audit_trail_recorder_v18_forensics_export() {
        let recorder = AuditTrailRecorderV18::new();
        let mut e1 = AuditTrailEventV18::new("access", "repo", "r1", "read");
        e1.actor_id = Some("user-1".into());
        e1.session_id = Some("sess-1".into());
        ...
        e1.risk_score = 50;
        e1.compliance_status = ComplianceStatusV18::Compliant;
        let mut e2 = AuditTrailEventV18::new("access", "repo", "r2", "write");
        e2.actor_id = Some("user-1".into());
        e2.session_id = Some("sess-1".into());
    ...
    #[test]
    fn test_audit_trail_recorder_v18_detect_anomalies() {
        let recorder = AuditTrailRecorderV18::new();
        for i in 0..20 {
            let mut e = AuditTrailEventV18::new("access", "repo", &format!("r{}", i), "read");
            e.actor_id = Some("user-1".into());
            e.risk_score = 10;
            e.compliance_status = ComplianceStatusV18::Compliant;
            recorder.record(e);
        }
        for i in 0..5 {
            let mut e = AuditTrailEventV18::new("access", "repo", &format!("r{}", i + 20), "read");
        ...
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV18::HighActivity));
    }

    #[test]
    fn test_audit_trail_recorder_v18_detect_anomalies_compliance_drift() {
        let recorder = AuditTrailRecorderV18::new();
        for i in 0..15 {
            let mut e = AuditTrailEventV18::new("security", "repo", &format!("r{}", i), "scan");
            e.actor_id = Some("user-1".into());
            e.compliance_status = ComplianceStatusV18::NonCompliant;
            recorder.record(e);
        }
        let anomalies = recorder.detect_anomalies();
        assert!(!anomalies.is_empty());
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV18::ComplianceDrift));
    }

    #[test]
    fn test_audit_trail_recorder_v18_detect_anomalies_unusual_access() {
        let recorder = AuditTrailRecorderV18::new();
        let mut e1 = AuditTrailEventV18::new("access", "repo", "r1", "read");
        e1.actor_id = Some("user-1".into());
        e1.ip_address = Some("10.0.0.1".into());
        recorder.record(e1);
        for i in 0..5 {
            let mut e = AuditTrailEventV18::new("access", "repo", &format!("r{}", i + 2), "read");
            e.actor_id = Some("user-1".into());
            e.ip_address = Some(format!("10.0.0.{}", i + 2));
            recorder.record(e);
        }
        let anomalies = recorder.detect_anomalies();
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV18::UnusualAccess));
    }
    ...
    #[test]
    fn test_audit_trail_builder_v18() {
        let geo = GeoLocationV18::new()
            .with_country("US")
            .with_city("New York");
        let event = AuditTrailBuilderV18::new("security", "repo", "r1", "scan")
            .actor_id("user-1")
            .ip_address("10.0.0.1")
            ...
            .geo_location(geo)
            .risk_score(75)
            .compliance_status(ComplianceStatusV18::Compliant)
        ...
        assert_eq!(event.risk_score, 75);
        assert_eq!(event.compliance_status, ComplianceStatusV18::Compliant);
        assert_eq!(event.details["key"], "value");
    }

    #[test]
    fn test_audit_trail_export_v18() {
        let recorder = AuditTrailRecorderV18::new();
        recorder.record(AuditTrailEventV18::new("access", "repo", "r1", "read"));
        let export = recorder.export(&AuditTrailQueryV18::default());
        assert_eq!(export.total_events, 1);
        assert_eq!(export.format, ExportFormatV18::Json);
    }

    #[test]
    fn test_record_compliance_event_v18() {
        let recorder = AuditTrailRecorderV18::new();
        record_compliance_event_v18(
            &recorder,
            "repo-1",
            "assessment_complete",
            "SOC 2",
        ...
        assert_eq!(results[0].compliance_status, ComplianceStatusV18::Compliant);
    }

    #[test]
    fn test_record_security_event_v18() {
        let recorder = AuditTrailRecorderV18::new();
        record_security_event_v18(
            &recorder,
            "repo-1",
            "scan_completed",
            "sast",
    ...
    fn test_risk_level_display_name() {
        assert_eq!(RiskLevelV18::Critical.display_name(), "Critical");
        assert_eq!(RiskLevelV18::Low.display_name(), "Low");
    }

    #[test]
    fn test_export_format_v18_serialization() {
        assert_eq!(
            serde_json::to_string(&ExportFormatV18::Json).unwrap(),
            "\"json\""
        );
        assert_eq!(
            serde_json::to_string(&ExportFormatV18::Csv).unwrap(),
            "\"csv\""
        );
    }

    #[test]
    fn test_anomaly_type_v18_serialization() {
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV18::HighActivity).unwrap(),
            "\"high_activity\""
        );
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV18::ComplianceDrift).unwrap(),
            "\"compliance_drift\""
        );
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV18::GeographicAnomaly).unwrap(),
            "\"geographic_anomaly\""
        );
    }

    #[test]
    fn test_risk_level_v18_serialization() {
        assert_eq!(
            serde_json::to_string(&RiskLevelV18::Critical).unwrap(),
            "\"critical\""
        );
        assert_eq!(
            serde_json::to_string(&RiskLevelV18::Low).unwrap(),
            "\"low\""
        );
    }
}
