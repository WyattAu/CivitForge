...
impl DbRepository {
    ...
    pub async fn update_user_profile(
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| CoreError::Database(format!("update_user_profile: {e}")))?;
        Ok(row)
    }
    ...
    pub async fn create_fork(
        ...
        let row = sqlx::query_as::<_, Repository>(
            r#"INSERT INTO repositories (name, description, owner_id, visibility, default_branch, is_fork, parent_repo_id, archived)
               VALUES ($1, $2, $3, $4, $5, true, $6, false)
               RETURNING id, name, description, owner_id, org_id, visibility, default_branch, is_fork, parent_repo_id, stars_count, watchers_count, archived, created_at, updated_at"#,
        )
        .bind(name)
        .bind(description)
        .bind(owner_id)
