...
pub struct SecurityScanRuleV17 {
    pub id: String,
    pub name: String,
    pub description: String,
    pub rule_type: RuleTypeV17,
    pub severity: SeverityV17,
    pub pattern: String,
    pub enabled: bool,
    pub version: u32,
    ...
    pub compliance_mapping: ComplianceMappingV17,
    pub created_at: DateTime<Utc>,
    pub updated_at: DateTime<Utc>,
}

impl SecurityScanRuleV17 {
    pub fn new(name: String, description: String, rule_type: RuleTypeV17, pattern: String) -> Self {
        let now = Utc::now();
        Self {
            ...
            compliance_mapping: ComplianceMappingV17::default(),
            created_at: now,
            updated_at: now,
        }
    }

    pub fn with_severity(mut self, severity: SeverityV17) -> Self {
    ...
    pub fn with_compliance_mapping(mut self, mapping: ComplianceMappingV17) -> Self {
...
pub struct ComplianceMappingV17 {
    pub mappings: Vec<ComplianceMappingEntryV17>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceMappingEntryV17 {
    pub framework_id: String,
    pub framework_name: String,
    pub requirement_id: String,
    pub mapping_type: ComplianceMappingTypeV17,
    pub confidence: f64,
...
#[serde(rename_all = "snake_case")]
pub enum ComplianceMappingTypeV17 {
    Direct,
    Partial,
    Advisory,
    Inherited,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScanFixV17 {
    pub id: String,
    pub scan_id: String,
    pub rule_id: String,
    pub file_path: String,
    pub line_number: Option<u32>,
    pub fix_type: FixTypeV17,
    pub fix_content: String,
    pub applied: bool,
    pub test_result: Option<FixTestResultV17>,
...
pub struct FixAnalyticsV17 {
    ...
    pub compliance_impact: ComplianceImpactV17,
    pub rollback_count: u64,
    pub avg_apply_time_ms: f64,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ComplianceImpactV17 {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RuleVersionHistoryV17 {
    pub rule_id: String,
    pub versions: Vec<RuleVersionEntryV17>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RuleVersionEntryV17 {
    pub version: u32,
    pub pattern: String,
    pub changed_by: Option<String>,
    pub changed_at: DateTime<Utc>,
    pub change_notes: Option<String>,
    pub compliance_mappings_snapshot: ComplianceMappingV17,
    pub breaking_change: bool,
}

#[derive(Debug, Clone)]
pub struct SecurityScanRuleManagerV17 {
    rules: Vec<SecurityScanRuleV17>,
    fixes: Vec<ScanFixV17>,
    version_history: HashMap<String, Vec<RuleVersionEntryV17>>,
}

impl Default for SecurityScanRuleManagerV17 {
    fn default() -> Self {
        Self::new()
    }
}

impl SecurityScanRuleManagerV17 {
    pub fn new() -> Self {
        Self {
            rules: Vec::new(),
            fixes: Vec::new(),
    ...
    pub fn add_rule(&mut self, rule: SecurityScanRuleV17) {
        let entry = RuleVersionEntryV17 {
    ...
    pub fn get_rule(&self, rule_id: &str) -> Option<&SecurityScanRuleV17> {
        self.rules.iter().find(|r| r.id == rule_id)
    }

    pub fn get_rule_mut(&mut self, rule_id: &str) -> Option<&mut SecurityScanRuleV17> {
        self.rules.iter_mut().find(|r| r.id == rule_id)
    }

    pub fn list_rules(&self) -> &[SecurityScanRuleV17] {
        &self.rules
    }

    pub fn list_enabled_rules(&self) -> Vec<&SecurityScanRuleV17> {
        self.rules.iter().filter(|r| r.enabled).collect()
    }

    pub fn list_auto_fix_rules(&self) -> Vec<&SecurityScanRuleV17> {
        self.rules.iter().filter(|r| r.auto_fix && r.enabled).collect()
    }

    pub fn list_rules_by_severity(&self, severity: SeverityV17) -> Vec<&SecurityScanRuleV17> {
        self.rules.iter().filter(|r| r.severity == severity).collect()
    }

    pub fn list_rules_with_compliance(&self) -> Vec<&SecurityScanRuleV17> {
        self.rules
            .iter()
    ...
    pub fn list_auto_verifiable_rules(&self) -> Vec<&SecurityScanRuleV17> {
        self.rules
            .iter()
            .filter(|r| {
                r.enabled
    ...
    pub fn get_version_history(&self, rule_id: &str) -> Option<&Vec<RuleVersionEntryV17>> {
        self.version_history.get(rule_id)
    }

    pub fn test_rule(&self, rule: &SecurityScanRuleV17, input: &str) -> RuleTestResultV17 {
        let start = std::time::Instant::now();
        let mut matched_lines = Vec::new();

        for (idx, line) in input.lines().enumerate() {
            let matched = match rule.rule_type {
                RuleTypeV17::Regex => regex::Regex::new(&rule.pattern)
                    .map(|re| re.is_match(line))
                    .unwrap_or(false),
                RuleTypeV17::Keyword => line.contains(&rule.pattern),
                RuleTypeV17::Pattern => line.to_lowercase().contains(&rule.pattern.to_lowercase()),
                RuleTypeV17::Ast | RuleTypeV17::Custom | RuleTypeV17::Composite => false,
            };
            if matched {
    ...
    pub fn record_fix(&mut self, fix: ScanFixV17) {
        self.fixes.push(fix);
    }
    ...
    pub fn test_fix(&mut self, fix_id: &str, test_input: &str) -> Result<FixTestResultV17, String> {
        let fix = self
            .fixes
            .iter_mut()
            .find(|f| f.id == fix_id)
    ...
    pub fn get_fixes_for_scan(&self, scan_id: &str) -> Vec<&ScanFixV17> {
        self.fixes.iter().filter(|f| f.scan_id == scan_id).collect()
    }

    pub fn get_pending_fixes(&self) -> Vec<&ScanFixV17> {
        self.fixes.iter().filter(|f| !f.applied).collect()
    }

    pub fn get_rollbackable_fixes(&self) -> Vec<&ScanFixV17> {
        self.fixes
            .iter()
    ...
    pub fn compute_fix_analytics(&self) -> FixAnalyticsV17 {
        ...
        let compliance_impact = ComplianceImpactV17 {
    ...
    pub fn generate_report(&self) -> String {
        let mut report = String::from("=== Security Scan V17 Report ===\n\n");
        ...
        for severity in &[
            SeverityV17::Critical,
            SeverityV17::High,
            SeverityV17::Medium,
            SeverityV17::Low,
            SeverityV17::Informational,
        ] {
...
pub fn create_default_scan_rules_v17() -> Vec<SecurityScanRuleV17> {
    vec![
        SecurityScanRuleV17::new(
            "Hardcoded Secret".into(),
            "Detects hardcoded secrets and API keys".into(),
            RuleTypeV17::Regex,
            r#"(?i)(api[_-]?key|secret|password|token)\s*[:=]\s*['"][^'"]+['"]"#.into(),
        )
        .with_severity(SeverityV17::Critical)
        .with_compliance_mapping(ComplianceMappingV17 {
            mappings: vec![ComplianceMappingEntryV17 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
                requirement_id: "CC6.1".into(),
                mapping_type: ComplianceMappingTypeV17::Direct,
                confidence: 0.95,
        ...
        .with_severity(SeverityV17::High)
        .with_compliance_mapping(ComplianceMappingV17 {
            mappings: vec![ComplianceMappingEntryV17 {
...
mod tests {
    ...
    fn sample_rule(id: &str, severity: SeverityV17, auto_fix: bool) -> SecurityScanRuleV17 {
        SecurityScanRuleV17 {
            id: id.to_string(),
            name: format!("Rule {id}"),
            description: format!("Description for rule {id}"),
            rule_type: RuleTypeV17::Pattern,
            severity,
    ...
    fn sample_fix(id: &str, scan_id: &str, rule_id: &str, applied: bool) -> ScanFixV17 {
        ScanFixV17 {
            id: id.to_string(),
            ...
            line_number: Some(42),
            fix_type: FixTypeV17::PatternReplace,
    ...
    #[test]
    fn test_add_rule() {
        let mut manager = SecurityScanRuleManagerV17::new();
        manager.add_rule(sample_rule("r1", SeverityV17::High, false));
    ...
    fn test_get_rules_by_severity() {
        let mut manager = SecurityScanRuleManagerV17::new();
        manager.add_rule(sample_rule("r1", SeverityV17::High, false));
        manager.add_rule(sample_rule("r2", SeverityV17::Critical, false));
        manager.add_rule(sample_rule("r3", SeverityV17::High, false));
        let high = manager.list_rules_by_severity(SeverityV17::High);
    ...
    fn test_get_auto_fix_rules() {
        let mut manager = SecurityScanRuleManagerV17::new();
        manager.add_rule(sample_rule("r1", SeverityV17::High, true));
        manager.add_rule(sample_rule("r2", SeverityV17::Critical, false));
    ...
    fn test_generate_report() {
        ...
        assert!(report.contains("Security Scan V17 Report"));
    ...
    fn test_compliance_mapping() {
        let rule = SecurityScanRuleV17::new(
            "Test".into(),
            "Desc".into(),
            RuleTypeV17::Regex,
            "pattern".into(),
    ...
    fn test_severity_risk_weight() {
        assert_eq!(SeverityV17::Critical.risk_weight(), 5);
        assert_eq!(SeverityV17::High.risk_weight(), 4);
        assert_eq!(SeverityV17::Medium.risk_weight(), 2);
        assert_eq!(SeverityV17::Low.risk_weight(), 1);
        assert_eq!(SeverityV17::Informational.risk_weight(), 0);
    }

    #[test]
    fn test_severity_display_name() {
        assert_eq!(SeverityV17::Critical.display_name(), "Critical");
        assert_eq!(SeverityV17::High.display_name(), "High");
    }

    #[test]
    fn test_empty_manager() {
        let manager = SecurityScanRuleManagerV17::new();
    ...
    fn test_fix_analytics_with_compliance() {
        let mut manager = SecurityScanRuleManagerV17::new();
        let mut rule = sample_rule("r1", SeverityV17::High, true);
        rule.compliance_mapping = ComplianceMappingV17 {
            mappings: vec![ComplianceMappingEntryV17 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
                requirement_id: "CC6.1".into(),
    ...
    fn test_default_rules_have_compliance_mapping() {
        let rules = create_default_scan_rules_v17();
    ...
    fn test_list_rules_with_compliance() {
        let mut manager = SecurityScanRuleManagerV17::new();
        let mut rule = sample_rule("r1", SeverityV17::High, false);
        rule.compliance_mapping = ComplianceMappingV17 {
            mappings: vec![ComplianceMappingEntryV17 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
        ...
        manager.add_rule(rule);
        manager.add_rule(sample_rule("r2", SeverityV17::Low, false));
    ...
    fn test_rule_risk_score() {
        let rule = sample_rule("r1", SeverityV17::Critical, false);
