...
pub struct SecurityScanRuleV14 {
    pub id: String,
    pub name: String,
    pub description: String,
    pub rule_type: RuleTypeV14,
    pub severity: SeverityV14,
    pub pattern: String,
    pub enabled: bool,
    pub version: u32,
    ...
    pub compliance_mapping: ComplianceMappingV14,
    pub created_at: DateTime<Utc>,
    pub updated_at: DateTime<Utc>,
}

impl SecurityScanRuleV14 {
    pub fn new(name: String, description: String, rule_type: RuleTypeV14, pattern: String) -> Self {
        let now = Utc::now();
        Self {
            ...
            compliance_mapping: ComplianceMappingV14::default(),
            created_at: now,
            updated_at: now,
        }
    }

    pub fn with_severity(mut self, severity: SeverityV14) -> Self {
    ...
    pub fn with_compliance_mapping(mut self, mapping: ComplianceMappingV14) -> Self {
...
pub struct ComplianceMappingV14 {
    pub mappings: Vec<ComplianceMappingEntryV14>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceMappingEntryV14 {
    pub framework_id: String,
    pub framework_name: String,
    pub requirement_id: String,
    pub mapping_type: ComplianceMappingTypeV14,
    pub confidence: f64,
...
#[serde(rename_all = "snake_case")]
pub enum ComplianceMappingTypeV14 {
    Direct,
    Partial,
    Advisory,
    Inherited,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScanFixV14 {
    pub id: String,
    pub scan_id: String,
    pub rule_id: String,
    pub file_path: String,
    pub line_number: Option<u32>,
    pub fix_type: FixTypeV14,
    pub fix_content: String,
    pub applied: bool,
    pub test_result: Option<FixTestResultV14>,
...
pub struct FixAnalyticsV14 {
    ...
    pub compliance_impact: ComplianceImpactV14,
    pub rollback_count: u64,
    pub avg_apply_time_ms: f64,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ComplianceImpactV14 {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RuleVersionHistoryV14 {
    pub rule_id: String,
    pub versions: Vec<RuleVersionEntryV14>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RuleVersionEntryV14 {
    pub version: u32,
    pub pattern: String,
    pub changed_by: Option<String>,
    pub changed_at: DateTime<Utc>,
    pub change_notes: Option<String>,
    pub compliance_mappings_snapshot: ComplianceMappingV14,
    pub breaking_change: bool,
}

#[derive(Debug, Clone)]
pub struct SecurityScanRuleManagerV14 {
    rules: Vec<SecurityScanRuleV14>,
    fixes: Vec<ScanFixV14>,
    version_history: HashMap<String, Vec<RuleVersionEntryV14>>,
}

impl Default for SecurityScanRuleManagerV14 {
    fn default() -> Self {
        Self::new()
    }
}

impl SecurityScanRuleManagerV14 {
    pub fn new() -> Self {
        Self {
            rules: Vec::new(),
            fixes: Vec::new(),
    ...
    pub fn add_rule(&mut self, rule: SecurityScanRuleV14) {
        let entry = RuleVersionEntryV14 {
    ...
    pub fn get_rule(&self, rule_id: &str) -> Option<&SecurityScanRuleV14> {
        self.rules.iter().find(|r| r.id == rule_id)
    }

    pub fn get_rule_mut(&mut self, rule_id: &str) -> Option<&mut SecurityScanRuleV14> {
        self.rules.iter_mut().find(|r| r.id == rule_id)
    }

    pub fn list_rules(&self) -> &[SecurityScanRuleV14] {
        &self.rules
    }

    pub fn list_enabled_rules(&self) -> Vec<&SecurityScanRuleV14> {
        self.rules.iter().filter(|r| r.enabled).collect()
    }

    pub fn list_auto_fix_rules(&self) -> Vec<&SecurityScanRuleV14> {
        self.rules.iter().filter(|r| r.auto_fix && r.enabled).collect()
    }

    pub fn list_rules_by_severity(&self, severity: SeverityV14) -> Vec<&SecurityScanRuleV14> {
        self.rules.iter().filter(|r| r.severity == severity).collect()
    }

    pub fn list_rules_with_compliance(&self) -> Vec<&SecurityScanRuleV14> {
        self.rules
            .iter()
    ...
    pub fn list_auto_verifiable_rules(&self) -> Vec<&SecurityScanRuleV14> {
        self.rules
            .iter()
            .filter(|r| {
                r.enabled
    ...
    pub fn get_version_history(&self, rule_id: &str) -> Option<&Vec<RuleVersionEntryV14>> {
        self.version_history.get(rule_id)
    }

    pub fn test_rule(&self, rule: &SecurityScanRuleV14, input: &str) -> RuleTestResultV14 {
        let start = std::time::Instant::now();
        let mut matched_lines = Vec::new();

        for (idx, line) in input.lines().enumerate() {
            let matched = match rule.rule_type {
                RuleTypeV14::Regex => regex::Regex::new(&rule.pattern)
                    .map(|re| re.is_match(line))
                    .unwrap_or(false),
                RuleTypeV14::Keyword => line.contains(&rule.pattern),
                RuleTypeV14::Pattern => line.to_lowercase().contains(&rule.pattern.to_lowercase()),
                RuleTypeV14::Ast | RuleTypeV14::Custom | RuleTypeV14::Composite => false,
            };
            if matched {
    ...
    pub fn record_fix(&mut self, fix: ScanFixV14) {
        self.fixes.push(fix);
    }
    ...
    pub fn test_fix(&mut self, fix_id: &str, test_input: &str) -> Result<FixTestResultV14, String> {
        let fix = self
            .fixes
            .iter_mut()
            .find(|f| f.id == fix_id)
    ...
    pub fn get_fixes_for_scan(&self, scan_id: &str) -> Vec<&ScanFixV14> {
        self.fixes.iter().filter(|f| f.scan_id == scan_id).collect()
    }

    pub fn get_pending_fixes(&self) -> Vec<&ScanFixV14> {
        self.fixes.iter().filter(|f| !f.applied).collect()
    }

    pub fn get_rollbackable_fixes(&self) -> Vec<&ScanFixV14> {
        self.fixes
            .iter()
    ...
    pub fn compute_fix_analytics(&self) -> FixAnalyticsV14 {
        ...
        let compliance_impact = ComplianceImpactV14 {
    ...
    pub fn generate_report(&self) -> String {
        let mut report = String::from("=== Security Scan V14 Report ===\n\n");
        ...
        for severity in &[
            SeverityV14::Critical,
            SeverityV14::High,
            SeverityV14::Medium,
            SeverityV14::Low,
            SeverityV14::Informational,
        ] {
...
pub fn create_default_scan_rules_v14() -> Vec<SecurityScanRuleV14> {
    vec![
        SecurityScanRuleV14::new(
            "Hardcoded Secret".into(),
            "Detects hardcoded secrets and API keys".into(),
            RuleTypeV14::Regex,
            r#"(?i)(api[_-]?key|secret|password|token)\s*[:=]\s*['"][^'"]+['"]"#.into(),
        )
        .with_severity(SeverityV14::Critical)
        .with_compliance_mapping(ComplianceMappingV14 {
            mappings: vec![ComplianceMappingEntryV14 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
                requirement_id: "CC6.1".into(),
                mapping_type: ComplianceMappingTypeV14::Direct,
                confidence: 0.95,
        ...
        .with_severity(SeverityV14::High)
        .with_compliance_mapping(ComplianceMappingV14 {
            mappings: vec![ComplianceMappingEntryV14 {
...
mod tests {
    ...
    fn sample_rule(id: &str, severity: SeverityV14, auto_fix: bool) -> SecurityScanRuleV14 {
        SecurityScanRuleV14 {
            id: id.to_string(),
            name: format!("Rule {id}"),
            description: format!("Description for rule {id}"),
            rule_type: RuleTypeV14::Pattern,
            severity,
    ...
    fn sample_fix(id: &str, scan_id: &str, rule_id: &str, applied: bool) -> ScanFixV14 {
        ScanFixV14 {
            id: id.to_string(),
            ...
            line_number: Some(42),
            fix_type: FixTypeV14::PatternReplace,
    ...
    #[test]
    fn test_add_rule() {
        let mut manager = SecurityScanRuleManagerV14::new();
        manager.add_rule(sample_rule("r1", SeverityV14::High, false));
    ...
    fn test_get_rules_by_severity() {
        let mut manager = SecurityScanRuleManagerV14::new();
        manager.add_rule(sample_rule("r1", SeverityV14::High, false));
        manager.add_rule(sample_rule("r2", SeverityV14::Critical, false));
        manager.add_rule(sample_rule("r3", SeverityV14::High, false));
        let high = manager.list_rules_by_severity(SeverityV14::High);
    ...
    fn test_get_auto_fix_rules() {
        let mut manager = SecurityScanRuleManagerV14::new();
        manager.add_rule(sample_rule("r1", SeverityV14::High, true));
        manager.add_rule(sample_rule("r2", SeverityV14::Critical, false));
    ...
    fn test_generate_report() {
        ...
        assert!(report.contains("Security Scan V14 Report"));
    ...
    fn test_compliance_mapping() {
        let rule = SecurityScanRuleV14::new(
            "Test".into(),
            "Desc".into(),
            RuleTypeV14::Regex,
            "pattern".into(),
    ...
    fn test_severity_risk_weight() {
        assert_eq!(SeverityV14::Critical.risk_weight(), 5);
        assert_eq!(SeverityV14::High.risk_weight(), 4);
        assert_eq!(SeverityV14::Medium.risk_weight(), 2);
        assert_eq!(SeverityV14::Low.risk_weight(), 1);
        assert_eq!(SeverityV14::Informational.risk_weight(), 0);
    }

    #[test]
    fn test_severity_display_name() {
        assert_eq!(SeverityV14::Critical.display_name(), "Critical");
        assert_eq!(SeverityV14::High.display_name(), "High");
    }

    #[test]
    fn test_empty_manager() {
        let manager = SecurityScanRuleManagerV14::new();
    ...
    fn test_fix_analytics_with_compliance() {
        let mut manager = SecurityScanRuleManagerV14::new();
        let mut rule = sample_rule("r1", SeverityV14::High, true);
        rule.compliance_mapping = ComplianceMappingV14 {
            mappings: vec![ComplianceMappingEntryV14 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
                requirement_id: "CC6.1".into(),
    ...
    fn test_default_rules_have_compliance_mapping() {
        let rules = create_default_scan_rules_v14();
    ...
    fn test_list_rules_with_compliance() {
        let mut manager = SecurityScanRuleManagerV14::new();
        let mut rule = sample_rule("r1", SeverityV14::High, false);
        rule.compliance_mapping = ComplianceMappingV14 {
            mappings: vec![ComplianceMappingEntryV14 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
        ...
        manager.add_rule(rule);
        manager.add_rule(sample_rule("r2", SeverityV14::Low, false));
    ...
    fn test_rule_risk_score() {
        let rule = sample_rule("r1", SeverityV14::Critical, false);
