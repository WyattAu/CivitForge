...
impl LogAggregationService {
    ...
    // V13: Log entries v13 and alert rules v10

    pub async fn ingest_v13(
        &self,
        input: CreateLogEntryV13,
    ) -> Result<LogEntryV13, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV13Row>(
            r#"INSERT INTO log_entries_v13 (level, message, source, service, trace_id, span_id, metadata, retention_days)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at"#,
        )
        .bind(input.level.to_string())
    ...
    pub async fn search_v13(
        &self,
        filter: LogSearchFilterV13,
    ) -> Result<LogSearchResultV13, sqlx::Error> {
        let limit = filter.limit.unwrap_or(100).min(1000);
        let offset = filter.offset.unwrap_or(0);
        let level_str = filter.level.map(|l| l.to_string());
        let search_pattern = filter.search.map(|s| format!("%{}%", s));
        ...
        let total_count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v13
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        let entries = sqlx::query_as::<_, LogEntryV13Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at FROM log_entries_v13
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(LogSearchResultV13 {
            entries: entries.into_iter().map(|r| r.into()).collect(),
            total_count,
        })
    }

    pub async fn get_by_id_v13(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogEntryV13>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV13Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v13 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn mark_indexed_v13(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("UPDATE log_entries_v13 SET indexed = true WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn delete_old_entries_v13(
        &self,
        max_age_days: i32,
    ) -> Result<i64, sqlx::Error> {
        let result = sqlx::query(
            "DELETE FROM log_entries_v13 WHERE created_at < NOW() - make_interval(days => $1::int)",
        )
        .bind(max_age_days)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn create_alert_rule_v10(
        &self,
        input: CreateLogAlertRuleV10,
    ) -> Result<LogAlertRuleV10, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV10Row>(
            r#"INSERT INTO log_alert_rules_v10 (name, level, pattern, threshold, window_seconds, enabled)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, level, pattern, threshold, window_seconds, enabled, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_alert_rule_v10(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogAlertRuleV10>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV10Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v10 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_alert_rules_v10(
        &self,
    ) -> Result<Vec<LogAlertRuleV10>, sqlx::Error> {
        let rows = sqlx::query_as::<_, LogAlertRuleV10Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v10 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(|r| r.into()).collect())
    ...
    pub async fn update_alert_rule_v10(
        &self,
        id: uuid::Uuid,
        input: UpdateLogAlertRuleV10,
    ) -> Result<LogAlertRuleV10, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV10Row>(
            r#"UPDATE log_alert_rules_v10 SET
             name = COALESCE($2, name),
             level = COALESCE($3, level),
             pattern = COALESCE($4, pattern),
             threshold = COALESCE($5, threshold),
    ...
    pub async fn delete_alert_rule_v10(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM log_alert_rules_v10 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn check_alert_rule_v10(
        &self,
        id: uuid::Uuid,
    ) -> Result<(LogAlertRuleV10, i64), sqlx::Error> {
        let rule = self.get_alert_rule_v10(id).await?.ok_or_else(|| sqlx::Error::RowNotFound)?;
        let count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v13
             WHERE level = $1 AND message ILIKE $2
             AND created_at >= NOW() - make_interval(seconds => $3::int)"#,
    ...
    pub async fn check_all_alert_rules_v10(
        &self,
    ) -> Result<Vec<(LogAlertRuleV10, i64, bool)>, sqlx::Error> {
        let rules = self.list_alert_rules_v10().await?;
        let mut results = Vec::new();
        for rule in rules {
            if !rule.enabled {
                continue;
    ...
    pub async fn match_pattern_v13(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<Vec<LogEntryV13>, sqlx::Error> {
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV13Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v13
             WHERE message ILIKE $1
             AND ($2::timestamptz IS NULL OR created_at >= $2)
             AND ($3::timestamptz IS NULL OR created_at <= $3)
             ORDER BY created_at DESC LIMIT 1000"#,
    ...
    pub async fn get_pattern_matches_v13(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<LogPatternMatchV13, sqlx::Error> {
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV13Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v13
...
impl From<LogEntryV13Row> for LogEntryV13 {
    fn from(row: LogEntryV13Row) -> Self {
        LogEntryV13 {
            id: row.id,
            level: row.level.parse().unwrap_or(LogLevel::Info),
            message: row.message,
...
impl From<LogAlertRuleV10Row> for LogAlertRuleV10 {
    fn from(row: LogAlertRuleV10Row) -> Self {
        LogAlertRuleV10 {
            id: row.id,
            name: row.name,
            level: row.level.parse().unwrap_or(LogLevel::Info),
...
impl From<LogEntryV14Row> for LogEntryV14 {
    fn from(row: LogEntryV14Row) -> Self {
        LogEntryV14 {
            id: row.id,
            level: row.level.parse().unwrap_or(LogLevel::Info),
            message: row.message,
...
impl From<LogAlertRuleV11Row> for LogAlertRuleV11 {
    fn from(row: LogAlertRuleV11Row) -> Self {
        LogAlertRuleV11 {
            id: row.id,
            name: row.name,
            level: row.level.parse().unwrap_or(LogLevel::Info),
...
impl From<LogEntryV15Row> for LogEntryV15 {
    fn from(row: LogEntryV15Row) -> Self {
        LogEntryV15 {
            id: row.id,
            level: row.level.parse().unwrap_or(LogLevel::Info),
            message: row.message,
...
impl From<LogAlertRuleV12Row> for LogAlertRuleV12 {
    fn from(row: LogAlertRuleV12Row) -> Self {
        LogAlertRuleV12 {
            id: row.id,
            name: row.name,
            level: row.level.parse().unwrap_or(LogLevel::Info),
...
impl LogAggregationService {
    // V14: Log entries with alert rules v11

    pub async fn ingest_v14(
        &self,
        input: CreateLogEntryV14,
    ) -> Result<LogEntryV14, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV14Row>(
            r#"INSERT INTO log_entries_v14 (level, message, source, service, trace_id, span_id, metadata, retention_days)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at"#,
        )
        .bind(input.level.to_string())
    ...
    pub async fn search_v14(
        &self,
        filter: LogSearchFilterV14,
    ) -> Result<LogSearchResultV14, sqlx::Error> {
        let limit = filter.limit.unwrap_or(100).min(1000);
        let offset = filter.offset.unwrap_or(0);
        let level_str = filter.level.map(|l| l.to_string());
        let search_pattern = filter.search.map(|s| format!("%{}%", s));
        ...
        let total_count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v14
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        let entries = sqlx::query_as::<_, LogEntryV14Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at FROM log_entries_v14
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(LogSearchResultV14 {
            entries: entries.into_iter().map(|r| r.into()).collect(),
            total_count,
        })
    }

    pub async fn get_by_id_v14(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogEntryV14>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV14Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v14 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn mark_indexed_v14(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("UPDATE log_entries_v14 SET indexed = true WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn delete_old_entries_v14(
        &self,
        max_age_days: i32,
    ) -> Result<i64, sqlx::Error> {
        let result = sqlx::query(
            "DELETE FROM log_entries_v14 WHERE created_at < NOW() - make_interval(days => $1::int)",
        )
        .bind(max_age_days)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn create_alert_rule_v11(
        &self,
        input: CreateLogAlertRuleV11,
    ) -> Result<LogAlertRuleV11, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV11Row>(
            r#"INSERT INTO log_alert_rules_v11 (name, level, pattern, threshold, window_seconds, enabled)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, level, pattern, threshold, window_seconds, enabled, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_alert_rule_v11(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogAlertRuleV11>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV11Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v11 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_alert_rules_v11(
        &self,
    ) -> Result<Vec<LogAlertRuleV11>, sqlx::Error> {
        let rows = sqlx::query_as::<_, LogAlertRuleV11Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v11 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(|r| r.into()).collect())
    ...
    pub async fn update_alert_rule_v11(
        &self,
        id: uuid::Uuid,
        input: UpdateLogAlertRuleV11,
    ) -> Result<LogAlertRuleV11, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV11Row>(
            r#"UPDATE log_alert_rules_v11 SET
             name = COALESCE($2, name),
             level = COALESCE($3, level),
             pattern = COALESCE($4, pattern),
             threshold = COALESCE($5, threshold),
    ...
    pub async fn delete_alert_rule_v11(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM log_alert_rules_v11 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn check_alert_rule_v11(
        &self,
        id: uuid::Uuid,
    ) -> Result<(LogAlertRuleV11, i64), sqlx::Error> {
        let rule = self.get_alert_rule_v11(id).await?.ok_or_else(|| sqlx::Error::RowNotFound)?;
        let count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v14
             WHERE level = $1 AND message ILIKE $2
             AND created_at >= NOW() - make_interval(seconds => $3::int)"#,
    ...
    pub async fn check_all_alert_rules_v11(
        &self,
    ) -> Result<Vec<(LogAlertRuleV11, i64, bool)>, sqlx::Error> {
        let rules = self.list_alert_rules_v11().await?;
        let mut results = Vec::new();
        for rule in rules {
            if !rule.enabled {
                continue;
    ...
    // V15: Log entries v15 and alert rules v12

    pub async fn ingest_v15(
        &self,
        input: CreateLogEntryV15,
    ) -> Result<LogEntryV15, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV15Row>(
            r#"INSERT INTO log_entries_v15 (level, message, source, service, trace_id, span_id, metadata, retention_days)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at"#,
        )
        .bind(input.level.to_string())
    ...
    pub async fn search_v15(
        &self,
        filter: LogSearchFilterV15,
    ) -> Result<LogSearchResultV15, sqlx::Error> {
        let limit = filter.limit.unwrap_or(100).min(1000);
        let offset = filter.offset.unwrap_or(0);
        let level_str = filter.level.map(|l| l.to_string());
        let search_pattern = filter.search.map(|s| format!("%{}%", s));
        ...
        let total_count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v15
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        let entries = sqlx::query_as::<_, LogEntryV15Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at FROM log_entries_v15
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(LogSearchResultV15 {
            entries: entries.into_iter().map(|r| r.into()).collect(),
            total_count,
        })
    }

    pub async fn get_by_id_v15(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogEntryV15>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV15Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v15 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn mark_indexed_v15(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("UPDATE log_entries_v15 SET indexed = true WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn delete_old_entries_v15(
        &self,
        max_age_days: i32,
    ) -> Result<i64, sqlx::Error> {
        let result = sqlx::query(
            "DELETE FROM log_entries_v15 WHERE created_at < NOW() - make_interval(days => $1::int)",
        )
        .bind(max_age_days)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn create_alert_rule_v12(
        &self,
        input: CreateLogAlertRuleV12,
    ) -> Result<LogAlertRuleV12, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV12Row>(
            r#"INSERT INTO log_alert_rules_v12 (name, level, pattern, threshold, window_seconds, enabled)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, level, pattern, threshold, window_seconds, enabled, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_alert_rule_v12(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogAlertRuleV12>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV12Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v12 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_alert_rules_v12(
        &self,
    ) -> Result<Vec<LogAlertRuleV12>, sqlx::Error> {
        let rows = sqlx::query_as::<_, LogAlertRuleV12Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v12 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(|r| r.into()).collect())
    ...
    pub async fn update_alert_rule_v12(
        &self,
        id: uuid::Uuid,
        input: UpdateLogAlertRuleV12,
    ) -> Result<LogAlertRuleV12, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV12Row>(
            r#"UPDATE log_alert_rules_v12 SET
             name = COALESCE($2, name),
             level = COALESCE($3, level),
             pattern = COALESCE($4, pattern),
             threshold = COALESCE($5, threshold),
    ...
    pub async fn delete_alert_rule_v12(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM log_alert_rules_v12 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn check_alert_rule_v12(
        &self,
        id: uuid::Uuid,
    ) -> Result<(LogAlertRuleV12, i64), sqlx::Error> {
        let rule = self.get_alert_rule_v12(id).await?.ok_or_else(|| sqlx::Error::RowNotFound)?;
        let count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v15
             WHERE level = $1 AND message ILIKE $2
             AND created_at >= NOW() - make_interval(seconds => $3::int)"#,
    ...
    pub async fn check_all_alert_rules_v12(
        &self,
    ) -> Result<Vec<(LogAlertRuleV12, i64, bool)>, sqlx::Error> {
        let rules = self.list_alert_rules_v12().await?;
        let mut results = Vec::new();
        for rule in rules {
            if !rule.enabled {
                continue;
    ...
    pub async fn match_pattern_v15(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<Vec<LogEntryV15>, sqlx::Error> {
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV15Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v15
             WHERE message ILIKE $1
             AND ($2::timestamptz IS NULL OR created_at >= $2)
             AND ($3::timestamptz IS NULL OR created_at <= $3)
             ORDER BY created_at DESC LIMIT 1000"#,
    ...
    pub async fn get_pattern_matches_v15(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<LogPatternMatchV15, sqlx::Error> {
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV15Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v15
    ...
    // V16: Log entries v16 and alert rules v13

    pub async fn ingest_v16(
        &self,
        input: CreateLogEntryV16,
    ) -> Result<LogEntryV16, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV16Row>(
            r#"INSERT INTO log_entries_v16 (level, message, source, service, trace_id, span_id, metadata, retention_days)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at"#,
        )
        .bind(input.level.to_string())
    ...
    pub async fn search_v16(
        &self,
        filter: LogSearchFilterV16,
    ) -> Result<LogSearchResultV16, sqlx::Error> {
        let limit = filter.limit.unwrap_or(100).min(1000);
        let offset = filter.offset.unwrap_or(0);
        let level_str = filter.level.map(|l| l.to_string());
        let search_pattern = filter.search.map(|s| format!("%{}%", s));
        ...
        let total_count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v16
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        let entries = sqlx::query_as::<_, LogEntryV16Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at FROM log_entries_v16
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(LogSearchResultV16 {
            entries: entries.into_iter().map(|r| r.into()).collect(),
            total_count,
        })
    }

    pub async fn get_by_id_v16(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogEntryV16>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV16Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v16 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn mark_indexed_v16(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("UPDATE log_entries_v16 SET indexed = true WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn delete_old_entries_v16(
        &self,
        max_age_days: i32,
    ) -> Result<i64, sqlx::Error> {
        let result = sqlx::query(
            "DELETE FROM log_entries_v16 WHERE created_at < NOW() - make_interval(days => $1::int)",
        )
        .bind(max_age_days)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn create_alert_rule_v13(
        &self,
        input: CreateLogAlertRuleV13,
    ) -> Result<LogAlertRuleV13, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV13Row>(
            r#"INSERT INTO log_alert_rules_v13 (name, level, pattern, threshold, window_seconds, enabled)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, level, pattern, threshold, window_seconds, enabled, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_alert_rule_v13(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogAlertRuleV13>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV13Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v13 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_alert_rules_v13(
        &self,
    ) -> Result<Vec<LogAlertRuleV13>, sqlx::Error> {
        let rows = sqlx::query_as::<_, LogAlertRuleV13Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v13 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(|r| r.into()).collect())
    ...
    pub async fn update_alert_rule_v13(
        &self,
        id: uuid::Uuid,
        input: UpdateLogAlertRuleV13,
    ) -> Result<LogAlertRuleV13, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV13Row>(
            r#"UPDATE log_alert_rules_v13 SET
             name = COALESCE($2, name),
             level = COALESCE($3, level),
             pattern = COALESCE($4, pattern),
             threshold = COALESCE($5, threshold),
    ...
    pub async fn delete_alert_rule_v13(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM log_alert_rules_v13 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn check_alert_rule_v13(
        &self,
        id: uuid::Uuid,
    ) -> Result<(LogAlertRuleV13, i64), sqlx::Error> {
        let rule = self.get_alert_rule_v13(id).await?.ok_or_else(|| sqlx::Error::RowNotFound)?;
        let count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v16
             WHERE level = $1 AND message ILIKE $2
             AND created_at >= NOW() - make_interval(seconds => $3::int)"#,
    ...
    pub async fn check_all_alert_rules_v13(
        &self,
    ) -> Result<Vec<(LogAlertRuleV13, i64, bool)>, sqlx::Error> {
        let rules = self.list_alert_rules_v13().await?;
        let mut results = Vec::new();
        for rule in rules {
            if !rule.enabled {
                continue;
    ...
    pub async fn match_pattern_v16(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<Vec<LogEntryV16>, sqlx::Error> {
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV16Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v16
             WHERE message ILIKE $1
             AND ($2::timestamptz IS NULL OR created_at >= $2)
             AND ($3::timestamptz IS NULL OR created_at <= $3)
             ORDER BY created_at DESC LIMIT 1000"#,
    ...
    pub async fn get_pattern_matches_v16(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<LogPatternMatchV16, sqlx::Error> {
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV16Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v16
    ...
    // V17: Log entries v17 and alert rules v14

    pub async fn ingest_v17(
        &self,
        input: CreateLogEntryV17,
    ) -> Result<LogEntryV17, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV17Row>(
            r#"INSERT INTO log_entries_v17 (level, message, source, service, trace_id, span_id, metadata, retention_days)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at"#,
        )
        .bind(input.level.to_string())
    ...
    pub async fn search_v17(
        &self,
        filter: LogSearchFilterV17,
    ) -> Result<LogSearchResultV17, sqlx::Error> {
        let limit = filter.limit.unwrap_or(100).min(1000);
        let offset = filter.offset.unwrap_or(0);
        let level_str = filter.level.map(|l| l.to_string());
        let search_pattern = filter.search.map(|s| format!("%{}%", s));
        ...
        let total_count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v17
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        let entries = sqlx::query_as::<_, LogEntryV17Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at FROM log_entries_v17
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(LogSearchResultV17 {
            entries: entries.into_iter().map(|r| r.into()).collect(),
            total_count,
        })
    }

    pub async fn get_by_id_v17(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogEntryV17>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV17Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v17 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn mark_indexed_v17(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("UPDATE log_entries_v17 SET indexed = true WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn delete_old_entries_v17(
        &self,
        max_age_days: i32,
    ) -> Result<i64, sqlx::Error> {
        let result = sqlx::query(
            "DELETE FROM log_entries_v17 WHERE created_at < NOW() - make_interval(days => $1::int)",
        )
        .bind(max_age_days)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn create_alert_rule_v14(
        &self,
        input: CreateLogAlertRuleV14,
    ) -> Result<LogAlertRuleV14, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV14Row>(
            r#"INSERT INTO log_alert_rules_v14 (name, level, pattern, threshold, window_seconds, enabled)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, level, pattern, threshold, window_seconds, enabled, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_alert_rule_v14(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogAlertRuleV14>, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV14Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v14 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_alert_rules_v14(
        &self,
    ) -> Result<Vec<LogAlertRuleV14>, sqlx::Error> {
        let rows = sqlx::query_as::<_, LogAlertRuleV14Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v14 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(|r| r.into()).collect())
    ...
    pub async fn update_alert_rule_v14(
        &self,
        id: uuid::Uuid,
        input: UpdateLogAlertRuleV14,
    ) -> Result<LogAlertRuleV14, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV14Row>(
            r#"UPDATE log_alert_rules_v14 SET
             name = COALESCE($2, name),
             level = COALESCE($3, level),
             pattern = COALESCE($4, pattern),
             threshold = COALESCE($5, threshold),
    ...
    pub async fn delete_alert_rule_v14(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM log_alert_rules_v14 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn check_alert_rule_v14(
        &self,
        id: uuid::Uuid,
    ) -> Result<(LogAlertRuleV14, i64), sqlx::Error> {
        let rule = self.get_alert_rule_v14(id).await?.ok_or_else(|| sqlx::Error::RowNotFound)?;
        let count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v17
             WHERE level = $1 AND message ILIKE $2
             AND created_at >= NOW() - make_interval(seconds => $3::int)"#,
    ...
    pub async fn check_all_alert_rules_v14(
        &self,
    ) -> Result<Vec<(LogAlertRuleV14, i64, bool)>, sqlx::Error> {
        let rules = self.list_alert_rules_v14().await?;
        let mut results = Vec::new();
        for rule in rules {
            if !rule.enabled {
                continue;
    ...
    pub async fn match_pattern_v17(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<Vec<LogEntryV17>, sqlx::Error> {
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV17Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v17
             WHERE message ILIKE $1
             AND ($2::timestamptz IS NULL OR created_at >= $2)
             AND ($3::timestamptz IS NULL OR created_at <= $3)
             ORDER BY created_at DESC LIMIT 1000"#,
    ...
    pub async fn get_pattern_matches_v17(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<LogPatternMatchV17, sqlx::Error> {
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV17Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v17
...
impl From<LogEntryV16Row> for LogEntryV16 {
    fn from(row: LogEntryV16Row) -> Self {
        LogEntryV16 {
            id: row.id,
            level: row.level.parse().unwrap_or(LogLevel::Info),
            message: row.message,
...
impl From<LogAlertRuleV13Row> for LogAlertRuleV13 {
    fn from(row: LogAlertRuleV13Row) -> Self {
        LogAlertRuleV13 {
            id: row.id,
            name: row.name,
            level: row.level.parse().unwrap_or(LogLevel::Info),
...
impl From<LogEntryV17Row> for LogEntryV17 {
    fn from(row: LogEntryV17Row) -> Self {
        LogEntryV17 {
            id: row.id,
            level: row.level.parse().unwrap_or(LogLevel::Info),
            message: row.message,
...
impl From<LogAlertRuleV14Row> for LogAlertRuleV14 {
    fn from(row: LogAlertRuleV14Row) -> Self {
        LogAlertRuleV14 {
            id: row.id,
            name: row.name,
            level: row.level.parse().unwrap_or(LogLevel::Info),
...
// V18: Log aggregation with pattern matching and threshold monitoring

#[derive(Debug, sqlx::FromRow)]
struct LogEntryV18Row {
    id: uuid::Uuid,
    level: String,
    message: String,
    source: String,
...
impl From<LogEntryV18Row> for LogEntryV18 {
    fn from(row: LogEntryV18Row) -> Self {
        LogEntryV18 {
            id: row.id,
            level: row.level.parse().unwrap_or(LogLevel::Info),
            message: row.message,
...
impl From<LogAlertRuleV15Row> for LogAlertRuleV15 {
    fn from(row: LogAlertRuleV15Row) -> Self {
        LogAlertRuleV15 {
            id: row.id,
            name: row.name,
            level: row.level.parse().unwrap_or(LogLevel::Info),
...
impl LogAggregationService {
    pub async fn ingest_v18(
        &self,
        input: CreateLogEntryV18,
    ) -> Result<LogEntryV18, sqlx::Error> {
        let row = sqlx::query_as::<_, LogEntryV18Row>(
            r#"INSERT INTO log_entries_v18 (level, message, source, service, trace_id, span_id, metadata, retention_days)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at"#,
        )
        .bind(input.level.to_string())
    ...
    pub async fn search_v18(
        &self,
        filter: LogSearchFilterV18,
    ) -> Result<LogSearchResultV18, sqlx::Error> {
        let limit = filter.limit.unwrap_or(100).min(1000);
        let offset = filter.offset.unwrap_or(0);
        ...
        let total_count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v18
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        let entries = sqlx::query_as::<_, LogEntryV18Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at FROM log_entries_v18
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        Ok(LogSearchResultV18 {
            entries: entries.into_iter().map(|r| r.into()).collect(),
            total_count,
        })
    }

    pub async fn create_alert_rule_v15(
        &self,
        input: CreateLogAlertRuleV15,
    ) -> Result<LogAlertRuleV15, sqlx::Error> {
        let row = sqlx::query_as::<_, LogAlertRuleV15Row>(
            r#"INSERT INTO log_alert_rules_v15 (name, level, pattern, threshold, window_seconds, enabled)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, level, pattern, threshold, window_seconds, enabled, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn list_alert_rules_v15(
        &self,
        enabled_only: Option<bool>,
    ) -> Result<Vec<LogAlertRuleV15>, sqlx::Error> {
        let rows = sqlx::query_as::<_, LogAlertRuleV15Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v15
    ...
    pub async fn match_patterns_v18(
        &self,
        pattern: &str,
        since: chrono::DateTime<chrono::Utc>,
    ) -> Result<LogPatternMatchV18, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct PatternCount {
            match_count: i64,
        }

        let result = sqlx::query_as::<_, PatternCount>(
            r#"SELECT COUNT(*) as match_count FROM log_entries_v18
             WHERE message ILIKE '%' || $1 || '%'
             AND created_at >= $2"#,
        )
        .bind(pattern)
        ...
        let first_match = sqlx::query_scalar::<_, chrono::DateTime<chrono::Utc>>(
            r#"SELECT MIN(created_at) FROM log_entries_v18
             WHERE message ILIKE '%' || $1 || '%'
             AND created_at >= $2"#,
        )
        .bind(pattern)
        ...
        let services = sqlx::query_as::<_, ServiceCount>(
            r#"SELECT DISTINCT service FROM log_entries_v18
             WHERE message ILIKE '%' || $1 || '%'
             AND created_at >= $2"#,
        )
        .bind(pattern)
        ...
        Ok(LogPatternMatchV18 {
            pattern: pattern.to_string(),
            match_count: result.match_count,
            first_match_at: first_match,
            last_match_at: last_match,
            affected_services: services.into_iter().map(|s| s.service).collect(),
        })
    }

    pub async fn check_threshold_alerts_v18(
        &self,
    ) -> Result<Vec<LogThresholdAlertV18>, sqlx::Error> {
        let rules = self.list_alert_rules_v15(Some(true)).await?;
        let mut alerts = Vec::new();

        for rule in rules {
            let window_start = chrono::Utc::now() - chrono::Duration::seconds(rule.window_seconds as i64);
            ...
            let count = sqlx::query_as::<_, CountResult>(
                r#"SELECT COUNT(*) as count FROM log_entries_v18
                 WHERE level = $1
                 AND message ILIKE '%' || $2 || '%'
                 AND created_at >= $3"#,
            )
            ...
            if count.count >= rule.threshold as i64 {
                alerts.push(LogThresholdAlertV18 {
    ...
    pub async fn ingest_v20(
        &self,
        input: CreateLogEntryV20,
    ) -> Result<LogEntryV20, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogEntryV20Row {
            id: uuid::Uuid,
            level: String,
            message: String,
            source: String,
        ...
        impl From<LogEntryV20Row> for LogEntryV20 {
            fn from(row: LogEntryV20Row) -> Self {
                LogEntryV20 {
                    id: row.id,
                    level: row.level.parse().unwrap_or(LogLevel::Info),
                    message: row.message,
        ...
        let row = sqlx::query_as::<_, LogEntryV20Row>(
            r#"INSERT INTO log_entries_v20 (level, message, source, service, trace_id, span_id, metadata, retention_days)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at"#,
        )
        .bind(input.level.to_string())
    ...
    pub async fn search_v20(
        &self,
        filter: LogSearchFilterV20,
    ) -> Result<LogSearchResultV20, sqlx::Error> {
        let limit = filter.limit.unwrap_or(100).min(1000);
        let offset = filter.offset.unwrap_or(0);
        let level_str = filter.level.map(|l| l.to_string());
        let search_pattern = filter.search.map(|s| format!("%{}%", s));
        ...
        let total_count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v20
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        let entries = sqlx::query_as::<_, LogEntryV20Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at FROM log_entries_v20
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(LogSearchResultV20 {
            entries: entries.into_iter().map(|r| r.into()).collect(),
            total_count,
        })
    }

    pub async fn get_by_id_v20(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogEntryV20>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogEntryV20Row {
            id: uuid::Uuid,
            level: String,
        ...
        let row = sqlx::query_as::<_, LogEntryV20Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v20 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn mark_indexed_v20(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("UPDATE log_entries_v20 SET indexed = true WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn delete_old_entries_v20(
        &self,
        max_age_days: i32,
    ) -> Result<i64, sqlx::Error> {
        let result = sqlx::query(
            "DELETE FROM log_entries_v20 WHERE created_at < NOW() - make_interval(days => $1::int)",
        )
        .bind(max_age_days)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn create_alert_rule_v17(
        &self,
        input: CreateLogAlertRuleV17,
    ) -> Result<LogAlertRuleV17, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogAlertRuleV17Row {
            id: uuid::Uuid,
            name: String,
            level: String,
            pattern: String,
        ...
        impl From<LogAlertRuleV17Row> for LogAlertRuleV17 {
            fn from(row: LogAlertRuleV17Row) -> Self {
                LogAlertRuleV17 {
                    id: row.id,
                    name: row.name,
                    level: row.level.parse().unwrap_or(LogLevel::Info),
        ...
        let row = sqlx::query_as::<_, LogAlertRuleV17Row>(
            r#"INSERT INTO log_alert_rules_v17 (name, level, pattern, threshold, window_seconds, enabled)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, level, pattern, threshold, window_seconds, enabled, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_alert_rule_v17(
        &self,
        id: uuid::Uuid,
    ) -> Result<Option<LogAlertRuleV17>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogAlertRuleV17Row {
            id: uuid::Uuid,
            name: String,
        ...
        let row = sqlx::query_as::<_, LogAlertRuleV17Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v17 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_alert_rules_v17(
        &self,
    ) -> Result<Vec<LogAlertRuleV17>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogAlertRuleV17Row {
            id: uuid::Uuid,
            name: String,
        ...
        let rows = sqlx::query_as::<_, LogAlertRuleV17Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v17 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.into_iter().map(|r| r.into()).collect())
    ...
    pub async fn update_alert_rule_v17(
        &self,
        id: uuid::Uuid,
        input: UpdateLogAlertRuleV17,
    ) -> Result<LogAlertRuleV17, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogAlertRuleV17Row {
            id: uuid::Uuid,
        ...
        let row = sqlx::query_as::<_, LogAlertRuleV17Row>(
            r#"UPDATE log_alert_rules_v17 SET
             name = COALESCE($2, name),
             level = COALESCE($3, level),
             pattern = COALESCE($4, pattern),
             threshold = COALESCE($5, threshold),
    ...
    pub async fn delete_alert_rule_v17(
        &self,
        id: uuid::Uuid,
    ) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM log_alert_rules_v17 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    pub async fn check_alert_rule_v17(
        &self,
        id: uuid::Uuid,
    ) -> Result<(LogAlertRuleV17, i64), sqlx::Error> {
        let rule = self.get_alert_rule_v17(id).await?.ok_or_else(|| sqlx::Error::RowNotFound)?;
        let count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v20
             WHERE level = $1 AND message ILIKE $2
             AND created_at >= NOW() - make_interval(seconds => $3::int)"#,
    ...
    pub async fn check_all_alert_rules_v17(
        &self,
    ) -> Result<Vec<(LogAlertRuleV17, i64, bool)>, sqlx::Error> {
    ...
    // V20: Pattern matching v20

    pub async fn match_pattern_v20(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<Vec<LogEntryV20>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogEntryV20Row {
            id: uuid::Uuid,
            level: String,
        ...
        let search_pattern = format!("%{}%", pattern);
        let rows = sqlx::query_as::<_, LogEntryV20Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at
             FROM log_entries_v20
             WHERE message ILIKE $1
             AND ($2::timestamptz IS NULL OR created_at >= $2)
             AND ($3::timestamptz IS NULL OR created_at <= $3)
             ORDER BY created_at DESC LIMIT 1000"#,
    ...
    pub async fn get_pattern_matches_v20(
        &self,
        pattern: &str,
        since: Option<chrono::DateTime<chrono::Utc>>,
        until: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<LogPatternMatchV20, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogEntryV20Row {
            id: uuid::Uuid,
            level: String,
    ...
    pub async fn get_threshold_alerts_v20(
        ...
        for rule in rules {
            ...
            if count >= rule.threshold as i64 {
                alerts.push(LogThresholdAlertV20 {
                    alert_rule_id: rule.id,
                    rule_name: rule.name,
                    current_count: count,
                    threshold: rule.threshold,
    ...
    pub async fn ingest_v21(
        &self,
        input: CreateLogEntryV21,
    ) -> Result<LogEntryV21, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogEntryV21Row {
            id: uuid::Uuid,
            level: String,
            message: String,
            source: String,
        ...
        impl From<LogEntryV21Row> for LogEntryV21 {
            fn from(row: LogEntryV21Row) -> Self {
                LogEntryV21 {
                    id: row.id,
                    level: row.level.parse().unwrap_or(LogLevel::Info),
                    message: row.message,
        ...
        let row = sqlx::query_as::<_, LogEntryV21Row>(
            r#"INSERT INTO log_entries_v21 (level, message, source, service, trace_id, span_id, metadata, retention_days)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at"#,
        )
        .bind(input.level.to_string())
    ...
    pub async fn search_v21(
        &self,
        filter: LogSearchFilterV21,
    ) -> Result<LogSearchResultV21, sqlx::Error> {
        let limit = filter.limit.unwrap_or(100).min(1000);
        let offset = filter.offset.unwrap_or(0);
        let level_str = filter.level.map(|l| l.to_string());
        let search_pattern = filter.search.map(|s| format!("%{}%", s));
        ...
        let total_count = sqlx::query_scalar::<_, i64>(
            r#"SELECT COUNT(*) FROM log_entries_v21
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        let entries = sqlx::query_as::<_, LogEntryV21Row>(
            r#"SELECT id, level, message, source, service, trace_id, span_id, metadata, retention_days, indexed, created_at FROM log_entries_v21
             WHERE ($1::text IS NULL OR level = $1)
             AND ($2::text IS NULL OR source = $2)
             AND ($3::text IS NULL OR service = $3)
             AND ($4::text IS NULL OR trace_id = $4)
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(LogSearchResultV21 {
            entries: entries.into_iter().map(|r| r.into()).collect(),
            total_count,
        })
    }

    pub async fn create_alert_rule_v18(
        &self,
        input: CreateLogAlertRuleV18,
    ) -> Result<LogAlertRuleV18, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogAlertRuleV18Row {
            id: uuid::Uuid,
            name: String,
            level: String,
            pattern: String,
        ...
        impl From<LogAlertRuleV18Row> for LogAlertRuleV18 {
            fn from(row: LogAlertRuleV18Row) -> Self {
                LogAlertRuleV18 {
                    id: row.id,
                    name: row.name,
                    level: row.level.parse().unwrap_or(LogLevel::Info),
        ...
        let row = sqlx::query_as::<_, LogAlertRuleV18Row>(
            r#"INSERT INTO log_alert_rules_v18 (name, level, pattern, threshold, window_seconds, enabled)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, level, pattern, threshold, window_seconds, enabled, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn list_alert_rules_v18(
        &self,
        enabled_only: Option<bool>,
    ) -> Result<Vec<LogAlertRuleV18>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct LogAlertRuleV18Row {
            id: uuid::Uuid,
            name: String,
        ...
        let rows = sqlx::query_as::<_, LogAlertRuleV18Row>(
            r#"SELECT id, name, level, pattern, threshold, window_seconds, enabled, created_at
             FROM log_alert_rules_v18
             WHERE ($1::bool IS NULL OR enabled = $1)
             ORDER BY created_at DESC"#,
        )
        .bind(enabled_only)
    ...
    pub async fn match_patterns_v21(
        &self,
        pattern: &str,
        since: chrono::DateTime<chrono::Utc>,
    ) -> Result<LogPatternMatchV21, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct PatternCount {
            match_count: i64,
        }
        let result = sqlx::query_as::<_, PatternCount>(
            r#"SELECT COUNT(*) as match_count FROM log_entries_v21
             WHERE message ILIKE '%' || $1 || '%'
             AND created_at >= $2"#,
        )
        .bind(pattern)
        ...
        let first_match = sqlx::query_scalar::<_, chrono::DateTime<chrono::Utc>>(
            r#"SELECT MIN(created_at) FROM log_entries_v21
             WHERE message ILIKE '%' || $1 || '%'
             AND created_at >= $2"#,
        )
        .bind(pattern)
        ...
        let services = sqlx::query_as::<_, ServiceCount>(
            r#"SELECT DISTINCT service FROM log_entries_v21
             WHERE message ILIKE '%' || $1 || '%'
             AND created_at >= $2"#,
        )
        .bind(pattern)
    ...
    pub async fn check_threshold_alerts_v21(
        &self,
    ) -> Result<Vec<LogThresholdAlertV21>, sqlx::Error> {
        let rules = self.list_alert_rules_v18(Some(true)).await?;
        let mut alerts = Vec::new();
        ...
        for rule in rules {
            ...
            let count = sqlx::query_as::<_, CountResult>(
                r#"SELECT COUNT(*) as count FROM log_entries_v21
                 WHERE level = $1
                 AND message ILIKE '%' || $2 || '%'
                 AND created_at >= $3"#,
            )
            ...
            if count.count >= rule.threshold as i64 {
                alerts.push(LogThresholdAlertV21 {
                    alert_rule_id: rule.id,
                    rule_name: rule.name,
                    current_count: count.count,
                    threshold: rule.threshold,
