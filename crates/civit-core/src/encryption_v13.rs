...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceCheckEntryV10 {
    pub id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyLifecycleManagementV10 {
    pub key_id: Uuid,
    pub key_name: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EncryptionAuditEntryV10 {
    pub id: Uuid,
    pub key_id: Uuid,
    pub action: String,
...
pub struct EncryptionV13Service {
    pool: PgPool,
}

impl EncryptionV13Service {
    ...
    pub async fn create_key_version(
        ...
        version: i32,
        key_material: &[u8],
    ) -> Result<KeyVersionEntryV10, sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct VersionRow {
            id: Uuid,
            key_id: Uuid,
        ...
        let row = sqlx::query_as::<_, VersionRow>(
            r#"INSERT INTO encryption_key_versions_v10 (key_id, version, key_material)
             VALUES ($1, $2, $3)
    ...
    pub async fn get_key_versions(
        &self,
        key_id: Uuid,
    ) -> Result<Vec<KeyVersionEntryV10>, sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct VersionRow {
            id: Uuid,
            key_id: Uuid,
        ...
        let rows = sqlx::query_as::<_, VersionRow>(
            r#"SELECT id, key_id, version, key_material, created_at
             FROM encryption_key_versions_v10
             WHERE key_id = $1
    ...
    pub async fn create_compliance_check(
        &self,
        check_type: &str,
    ) -> Result<ComplianceCheckEntryV10, sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct CheckRow {
            id: Uuid,
            check_type: String,
        ...
        let row = sqlx::query_as::<_, CheckRow>(
            r#"INSERT INTO encryption_compliance_checks_v10 (check_type)
             VALUES ($1)
    ...
    pub async fn update_compliance_check(
        ...
        let row = sqlx::query_as::<_, CheckRow>(
            r#"UPDATE encryption_compliance_checks_v10
    ...
    ) -> Result<Vec<ComplianceCheckEntryV10>, sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct CheckRow {
            id: Uuid,
            check_type: String,
            ...
            Some(ct) => {
                sqlx::query_as::<_, CheckRow>(
                    r#"SELECT id, check_type, status, findings, score, created_at
                     FROM encryption_compliance_checks_v10
                     WHERE check_type = $1
    ...
    pub async fn get_key_lifecycle_management(
        &self,
        key_id: Uuid,
    ) -> Result<Option<KeyLifecycleManagementV10>, sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct KeyRow {
            id: Uuid,
            name: String,
        ...
        if let Some(r) = row {
            let versions_count: (i64,) = sqlx::query_as(
                r#"SELECT COUNT(*) FROM encryption_key_versions_v10 WHERE key_id = $1"#,
            )
            .bind(key_id)
            .fetch_one(&self.pool)
            .await?;
            ...
            let compliance_score: (i32,) = sqlx::query_as(
                r#"SELECT COALESCE(AVG(score), 0) FROM encryption_compliance_checks_v10"#,
            )
            .fetch_one(&self.pool)
            .await?;

            let latest_version: Option<(Option<i32>,)> = sqlx::query_as(
                r#"SELECT MAX(version) FROM encryption_key_versions_v10 WHERE key_id = $1"#,
            )
            .bind(key_id)
            .fetch_optional(&self.pool)
            .await?;
            ...
            Ok(Some(KeyLifecycleManagementV10 {
                key_id: r.id,
                key_name: r.name,
    ...
    ) -> Result<EncryptionAuditEntryV10, sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct AuditRow {
            id: Uuid,
            key_id: Uuid,
