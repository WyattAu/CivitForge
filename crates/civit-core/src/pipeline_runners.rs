//! Pipeline Runners: Advanced runner management with metrics collection,
...
impl From<RunnerRow> for PipelineRunner {
...
pub struct PipelineRunnersService {
    pool: PgPool,
}

impl PipelineRunnersService {
    ...
    pub async fn list_runners(
        ...
        status: Option<&str>,
        tags: Option<&[String]>,
    ) -> Result<Vec<PipelineRunner>, sqlx::Error> {
    ...
    pub async fn get_runner(&self, id: Uuid) -> Result<Option<PipelineRunner>, sqlx::Error> {
    ...
    ) -> Result<PipelineRunner, sqlx::Error> {
    ...
    pub async fn find_available_runners(
        ...
        let runners: Vec<PipelineRunner> = rows.into_iter().map(|r| r.into()).collect();
...
mod tests {
    ...
    #[test]
    fn test_pipeline_runner_serialize() {
