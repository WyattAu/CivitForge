...
impl Default for GeoLocationV19 {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct AuditTrailEventV19 {
    ...
    pub geo_location: Option<GeoLocationV19>,
    pub risk_score: u32,
    pub compliance_status: ComplianceStatusV19,
    pub created_at: DateTime<Utc>,
}

impl AuditTrailEventV19 {
    pub fn new(
        event_type: &str,
        resource_type: &str,
        resource_id: &str,
        ...
        Self {
            ...
            geo_location: None,
            risk_score: 0,
            compliance_status: ComplianceStatusV19::Unknown,
            created_at: Utc::now(),
        }
    }
    ...
    pub fn with_compliance_status(mut self, status: ComplianceStatusV19) -> Self {
    ...
    pub fn is_compliant(&self) -> bool {
        self.compliance_status == ComplianceStatusV19::Compliant
    }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditTrailQueryV19 {
    ...
    pub request_id: Option<String>,
    pub session_id: Option<String>,
    pub compliance_status: Option<ComplianceStatusV19>,
...
impl Default for AuditTrailQueryV19 {
    fn default() -> Self {
        Self {
            event_type: None,
            resource_type: None,
...
impl AuditTrailQueryV19 {
    pub fn matches(&self, event: &AuditTrailEventV19) -> bool {
...
pub struct AuditTrailRecorderV19 {
    events: std::sync::Mutex<Vec<AuditTrailEventV19>>,
}

impl AuditTrailRecorderV19 {
    pub fn new() -> Self {
        Self {
    ...
    pub fn record(&self, event: AuditTrailEventV19) {
        let mut events = self.events.lock().unwrap();
        events.push(event);
    }

    pub fn search(&self, query: &AuditTrailQueryV19) -> Vec<AuditTrailEventV19> {
    ...
    pub fn get_session_events(&self, session_id: &str) -> Vec<AuditTrailEventV19> {
        let events = self.events.lock().unwrap();
        events
            .iter()
            .filter(|e| e.session_id.as_deref() == Some(session_id))
    ...
    pub fn get_request_events(&self, request_id: &str) -> Vec<AuditTrailEventV19> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn get_events_by_geo_country(&self, country: &str) -> Vec<AuditTrailEventV19> {
        let events = self.events.lock().unwrap();
        events
            .iter()
            .filter(|e| {
    ...
    pub fn get_events_by_compliance_status(&self, status: ComplianceStatusV19) -> Vec<AuditTrailEventV19> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn get_high_risk_events(&self, threshold: u32) -> Vec<AuditTrailEventV19> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn compliance_audit(
        ...
        let compliant_count = relevant
            .iter()
            .filter(|e| e.compliance_status == ComplianceStatusV19::Compliant)
            .count() as u32;
    ...
    pub fn detect_anomalies(&self) -> Vec<AnomalyDetectionResultV19> {
        let events = self.events.lock().unwrap();
        ...
        for (actor, count) in &actor_event_counts {
            if *count as f64 > avg_events_per_actor * 2.0 && *count > 10 {
                anomalies.push(AnomalyDetectionResultV19 {
                    anomaly_type: AnomalyTypeV19::HighActivity,
        ...
        for event in events.iter() {
            if event.compliance_status == ComplianceStatusV19::NonCompliant {
                failed_count += 1;
            }
            total_count += 1;
        }
        ...
        if total_count > 10 && (failed_count as f64 / total_count as f64) > 0.3 {
            anomalies.push(AnomalyDetectionResultV19 {
                anomaly_type: AnomalyTypeV19::ComplianceDrift,
                actor_id: None,
                description: format!(
        ...
        for (actor, ips) in &actor_ips {
            if ips.len() > 3 {
                anomalies.push(AnomalyDetectionResultV19 {
                    anomaly_type: AnomalyTypeV19::UnusualAccess,
                    actor_id: Some(actor.clone()),
                    description: format!(
    ...
    pub fn risk_assessment(&self) -> RiskAssessmentResultV19 {
        let events = self.events.lock().unwrap();
    ...
    pub fn export(&self, query: &AuditTrailQueryV19) -> AuditExportV19 {
        let events = self.search(query);
        AuditExportV19 {
            format: ExportFormatV19::Json,
...
impl Default for AuditTrailRecorderV19 {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceAuditResultV19 {
    pub event_type: String,
...
pub struct ForensicsExportV19 {
    ...
    pub events: Vec<AuditTrailEventV19>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditExportV19 {
    pub format: ExportFormatV19,
...
#[serde(rename_all = "snake_case")]
pub enum ExportFormatV19 {
    Json,
    Csv,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditTrailBuilderV19 {
    event_type: String,
    ...
    geo_location: Option<GeoLocationV19>,
    risk_score: u32,
    compliance_status: ComplianceStatusV19,
}

impl AuditTrailBuilderV19 {
    ...
    pub fn geo_location(mut self, geo: GeoLocationV19) -> Self {
    ...
    pub fn compliance_status(mut self, status: ComplianceStatusV19) -> Self {
        self.compliance_status = status;
        self
    }

    pub fn build(self) -> AuditTrailEventV19 {
        AuditTrailEventV19 {
            id: uuid::Uuid::new_v4().to_string(),
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AnomalyDetectionResultV19 {
    pub anomaly_type: AnomalyTypeV19,
    pub actor_id: Option<String>,
    pub description: String,
    pub risk_level: RiskLevelV19,
    pub detected_at: DateTime<Utc>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RiskAssessmentResultV19 {
    pub total_events: u32,
...
pub fn record_compliance_event_v18(
    recorder: &AuditTrailRecorderV19,
    repo_id: &str,
    action: &str,
    framework_name: &str,
    details: serde_json::Value,
    ...
    let event = AuditTrailBuilderV19::new("compliance", "repository", repo_id, action)
...
pub fn record_security_event_v18(
    ...
    let event = AuditTrailBuilderV19::new("security", "repository", repo_id, action)
        .details(serde_json::json!({
            "scan_type": scan_type,
            "details": details,
        }))
...
mod tests {
    ...
    fn make_event(event_type: &str, resource_type: &str, resource_id: &str, action: &str) -> AuditTrailEventV19 {
        AuditTrailEventV19 {
            id: uuid::Uuid::new_v4().to_string(),
            event_type: event_type.into(),
            resource_type: resource_type.into(),
    ...
    fn test_audit_trail_event_v18_new() {
        let event = AuditTrailEventV19::new("access", "repo", "r1", "read");
        ...
        assert_eq!(event.compliance_status, ComplianceStatusV19::Unknown);
    }
    ...
    fn test_audit_trail_event_v18_with_risk_score() {
        let event = AuditTrailEventV19::new("access", "repo", "r1", "read")
            .with_risk_score(75)
            .with_compliance_status(ComplianceStatusV19::NonCompliant);
        assert_eq!(event.risk_score, 75);
        assert_eq!(event.compliance_status, ComplianceStatusV19::NonCompliant);
    }
    ...
    fn test_event_helpers() {
        let event = AuditTrailEventV19::new("access", "repo", "r1", "read")
            .with_risk_score(80)
            .with_compliance_status(ComplianceStatusV19::Compliant);
    ...
    fn test_compliance_status_risk_contribution() {
        assert_eq!(ComplianceStatusV19::Compliant.risk_contribution(), 0);
        assert_eq!(ComplianceStatusV19::Partial.risk_contribution(), 50);
        assert_eq!(ComplianceStatusV19::NonCompliant.risk_contribution(), 100);
        assert_eq!(ComplianceStatusV19::Unknown.risk_contribution(), 25);
    }

    #[test]
    fn test_compliance_status_display_name() {
        assert_eq!(ComplianceStatusV19::Compliant.display_name(), "Compliant");
        assert_eq!(ComplianceStatusV19::NonCompliant.display_name(), "Non-Compliant");
    }

    #[test]
    fn test_audit_trail_query_v18_matches() {
        let mut event = AuditTrailEventV19::new("access", "repo", "r1", "read");
        event.request_id = Some("req-1".into());
        event.session_id = Some("sess-1".into());
        event.risk_score = 50;
        event.compliance_status = ComplianceStatusV19::Compliant;
        let query = AuditTrailQueryV19 {
            request_id: Some("req-1".into()),
            ..AuditTrailQueryV19::default()
        };
        assert!(query.matches(&event));
        let query2 = AuditTrailQueryV19 {
    ...
    fn test_audit_trail_query_v18_matches_compliance_status() {
        ...
        event.compliance_status = ComplianceStatusV19::Compliant;
        let query = AuditTrailQueryV19 {
            compliance_status: Some(ComplianceStatusV19::Compliant),
            ..AuditTrailQueryV19::default()
        };
        assert!(query.matches(&event));
        let query2 = AuditTrailQueryV19 {
            compliance_status: Some(ComplianceStatusV19::NonCompliant),
            ..AuditTrailQueryV19::default()
        };
        assert!(!query2.matches(&event));
    }
    ...
    fn test_audit_trail_recorder_v18_record_and_search() {
        let recorder = AuditTrailRecorderV19::new();
        let event = AuditTrailEventV19::new("access", "repo", "r1", "read");
        recorder.record(event);
        assert_eq!(recorder.count(), 1);
        let results = recorder.search(&AuditTrailQueryV19::default());
    ...
    fn test_audit_trail_recorder_v18_session_tracking() {
        let recorder = AuditTrailRecorderV19::new();
        let mut e1 = AuditTrailEventV19::new("access", "repo", "r1", "read");
        e1.session_id = Some("sess-1".into());
        let mut e2 = AuditTrailEventV19::new("access", "repo", "r1", "read");
        e2.session_id = Some("sess-1".into());
        let mut e3 = AuditTrailEventV19::new("access", "repo", "r1", "read");
    ...
    fn test_audit_trail_recorder_v18_geo_tracking() {
        let recorder = AuditTrailRecorderV19::new();
        let mut e1 = AuditTrailEventV19::new("access", "repo", "r1", "read");
        e1.geo_location = Some(GeoLocationV19::new().with_country("US"));
        let mut e2 = AuditTrailEventV19::new("access", "repo", "r2", "read");
        e2.geo_location = Some(GeoLocationV19::new().with_country("UK"));
        let mut e3 = AuditTrailEventV19::new("access", "repo", "r3", "read");
        e3.geo_location = Some(GeoLocationV19::new().with_country("US"));
        recorder.record(e1);
        recorder.record(e2);
        recorder.record(e3);
    ...
    fn test_audit_trail_recorder_v18_compliance_tracking() {
        let recorder = AuditTrailRecorderV19::new();
        let mut e1 = AuditTrailEventV19::new("access", "repo", "r1", "read");
        e1.compliance_status = ComplianceStatusV19::Compliant;
        let mut e2 = AuditTrailEventV19::new("access", "repo", "r2", "read");
        e2.compliance_status = ComplianceStatusV19::NonCompliant;
        let mut e3 = AuditTrailEventV19::new("access", "repo", "r3", "read");
        e3.compliance_status = ComplianceStatusV19::Compliant;
        recorder.record(e1);
        recorder.record(e2);
        recorder.record(e3);
        assert_eq!(recorder.get_events_by_compliance_status(ComplianceStatusV19::Compliant).len(), 2);
        assert_eq!(recorder.get_events_by_compliance_status(ComplianceStatusV19::NonCompliant).len(), 1);
    }
    ...
    fn test_audit_trail_recorder_v18_compliance_audit() {
        let recorder = AuditTrailRecorderV19::new();
        let mut e1 = AuditTrailEventV19::new("security", "repo", "r1", "scan");
        e1.actor_id = Some("user-1".into());
        e1.risk_score = 75;
        e1.compliance_status = ComplianceStatusV19::Compliant;
        let mut e2 = AuditTrailEventV19::new("security", "repo", "r2", "scan");
        e2.actor_id = Some("user-1".into());
        e2.risk_score = 25;
        e2.compliance_status = ComplianceStatusV19::NonCompliant;
        let mut e3 = AuditTrailEventV19::new("security", "repo", "r1", "fix");
    ...
    fn test_audit_trail_recorder_v18_forensics_export() {
        ...
        e1.risk_score = 50;
        e1.compliance_status = ComplianceStatusV19::Compliant;
        let mut e2 = AuditTrailEventV19::new("access", "repo", "r2", "write");
        e2.actor_id = Some("user-1".into());
        e2.session_id = Some("sess-1".into());
    ...
    fn test_audit_trail_recorder_v18_detect_anomalies() {
        let recorder = AuditTrailRecorderV19::new();
        for i in 0..20 {
            let mut e = AuditTrailEventV19::new("access", "repo", &format!("r{}", i), "read");
            e.actor_id = Some("user-1".into());
            e.risk_score = 10;
            e.compliance_status = ComplianceStatusV19::Compliant;
            recorder.record(e);
        }
        for i in 0..5 {
            let mut e = AuditTrailEventV19::new("access", "repo", &format!("r{}", i + 20), "read");
        ...
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV19::HighActivity));
    }
    ...
    fn test_audit_trail_recorder_v18_detect_anomalies_compliance_drift() {
        let recorder = AuditTrailRecorderV19::new();
        for i in 0..15 {
            let mut e = AuditTrailEventV19::new("security", "repo", &format!("r{}", i), "scan");
            e.actor_id = Some("user-1".into());
            e.compliance_status = ComplianceStatusV19::NonCompliant;
            recorder.record(e);
        }
        let anomalies = recorder.detect_anomalies();
        assert!(!anomalies.is_empty());
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV19::ComplianceDrift));
    }
    ...
    fn test_audit_trail_recorder_v18_detect_anomalies_unusual_access() {
        ...
        recorder.record(e1);
        for i in 0..5 {
            let mut e = AuditTrailEventV19::new("access", "repo", &format!("r{}", i + 2), "read");
            e.actor_id = Some("user-1".into());
            e.ip_address = Some(format!("10.0.0.{}", i + 2));
            recorder.record(e);
        }
        let anomalies = recorder.detect_anomalies();
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV19::UnusualAccess));
    }
    ...
    fn test_audit_trail_builder_v18() {
        let geo = GeoLocationV19::new()
            .with_country("US")
            .with_city("New York");
        let event = AuditTrailBuilderV19::new("security", "repo", "r1", "scan")
            .actor_id("user-1")
            .ip_address("10.0.0.1")
            ...
            .geo_location(geo)
            .risk_score(75)
            .compliance_status(ComplianceStatusV19::Compliant)
        ...
        assert_eq!(event.risk_score, 75);
        assert_eq!(event.compliance_status, ComplianceStatusV19::Compliant);
    ...
    fn test_audit_trail_export_v18() {
        let recorder = AuditTrailRecorderV19::new();
        recorder.record(AuditTrailEventV19::new("access", "repo", "r1", "read"));
        let export = recorder.export(&AuditTrailQueryV19::default());
        assert_eq!(export.total_events, 1);
        assert_eq!(export.format, ExportFormatV19::Json);
    }
    ...
    fn test_record_compliance_event_v18() {
        ...
        assert_eq!(results[0].compliance_status, ComplianceStatusV19::Compliant);
    }
    ...
    fn test_risk_level_display_name() {
        assert_eq!(RiskLevelV19::Critical.display_name(), "Critical");
        assert_eq!(RiskLevelV19::Low.display_name(), "Low");
    }

    #[test]
    fn test_export_format_v18_serialization() {
        assert_eq!(
            serde_json::to_string(&ExportFormatV19::Json).unwrap(),
            "\"json\""
        );
        assert_eq!(
            serde_json::to_string(&ExportFormatV19::Csv).unwrap(),
            "\"csv\""
        );
    }
    ...
    fn test_anomaly_type_v18_serialization() {
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV19::HighActivity).unwrap(),
            "\"high_activity\""
        );
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV19::ComplianceDrift).unwrap(),
            "\"compliance_drift\""
        );
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV19::GeographicAnomaly).unwrap(),
            "\"geographic_anomaly\""
        );
    }
    ...
    fn test_risk_level_v18_serialization() {
        assert_eq!(
            serde_json::to_string(&RiskLevelV19::Critical).unwrap(),
            "\"critical\""
        );
        assert_eq!(
            serde_json::to_string(&RiskLevelV19::Low).unwrap(),
            "\"low\""
        );
    }
}
