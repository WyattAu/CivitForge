...
impl From<WorkflowTemplateV8Row> for WorkflowTemplateV8 {
    fn from(row: WorkflowTemplateV8Row) -> Self {
        WorkflowTemplateV8 {
            id: row.id,
            name: row.name,
            description: row.description,
...
impl From<WorkflowTemplateV10Row> for WorkflowTemplateV10 {
    fn from(row: WorkflowTemplateV10Row) -> Self {
        WorkflowTemplateV10 {
            id: row.id,
            name: row.name,
            description: row.description,
...
#[derive(Debug, sqlx::FromRow)]
struct WorkflowTemplateReviewV9Row {
    id: Uuid,
    template_id: Uuid,
    user_id: Uuid,
    rating: i32,
...
impl From<WorkflowTemplateReviewV9Row> for WorkflowTemplateReviewV9 {
    fn from(row: WorkflowTemplateReviewV9Row) -> Self {
        WorkflowTemplateReviewV9 {
            id: row.id,
            template_id: row.template_id,
            user_id: row.user_id,
            rating: row.rating,
...
impl From<WorkflowTemplateV18Row> for WorkflowTemplateV18 {
    fn from(row: WorkflowTemplateV18Row) -> Self {
        WorkflowTemplateV18 {
            id: row.id,
            name: row.name,
            description: row.description,
...
#[derive(Debug, sqlx::FromRow)]
struct WorkflowTemplateReviewV17Row {
    id: Uuid,
    template_id: Uuid,
    user_id: Uuid,
    rating: i32,
...
impl WorkflowService {
    ...
    // --- V11: Workflow Template V8 with Reviews V7, Marketplace V8, Analytics V8, Recommendations V8 ---

    pub async fn create_template_v8(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV8, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"INSERT INTO workflow_templates_v8 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v8(&self, id: Uuid) -> Result<Option<WorkflowTemplateV8>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v8 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v8(&self) -> Result<Vec<WorkflowTemplateV8>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v8 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v8(&self) -> Result<Vec<WorkflowTemplateV8>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v8 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v8_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV8>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v8 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v8(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"UPDATE workflow_templates_v8 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v8(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v8 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v8_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV7Row>(
            r#"INSERT INTO workflow_template_reviews_v7 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v8_rating(template_id).await?;

        Ok(row.into())
    }

    async fn recalculate_template_v8_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v7 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v8 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v8_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV7, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV7Row>(
            r#"UPDATE workflow_template_reviews_v7 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v8_reviews(
        &self,
        template_id: Uuid,
    ...
    pub async fn record_template_v8_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v8 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v8_analytics(
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v7 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v8 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_marketplace_templates_v8(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v8 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v8(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
    ...
    // --- V12: Workflow Template V9 with Reviews V8, Marketplace V9, Analytics V9, Recommendations V9 ---
    ...
    pub async fn create_template_v9(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"INSERT INTO workflow_templates_v9 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v9(&self, id: Uuid) -> Result<Option<WorkflowTemplateV8>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v9 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v9(&self) -> Result<Vec<WorkflowTemplateV8>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v9 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v9(&self) -> Result<Vec<WorkflowTemplateV8>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v9 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v9_by_type(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v9 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v9(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"UPDATE workflow_templates_v9 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v9(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v9 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    async fn recalculate_template_v9_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v9 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn record_template_v9_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v9 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_marketplace_templates_v9(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV8Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v9 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v9(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
    ...
    // --- V13: Workflow Template V10 with Reviews V9, Marketplace V10, Analytics V10, Recommendations V10 ---

    pub async fn create_template_v10(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV10, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV10Row>(
            r#"INSERT INTO workflow_templates_v10 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v10(&self, id: Uuid) -> Result<Option<WorkflowTemplateV10>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV10Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v10 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v10(&self) -> Result<Vec<WorkflowTemplateV10>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV10Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v10 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v10(&self) -> Result<Vec<WorkflowTemplateV10>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV10Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v10 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v10_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV10>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV10Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v10 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v10(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV10Row>(
            r#"UPDATE workflow_templates_v10 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v10(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v10 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v10_review(
        ...
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV9, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV9Row>(
    ...
    async fn recalculate_template_v10_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v10 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v10_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV9>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateReviewV9Row>(
            r#"SELECT id, template_id, user_id, rating, review, helpful_count, created_at
             FROM workflow_template_reviews_v9 WHERE template_id = $1
             ORDER BY helpful_count DESC, created_at DESC"#,
        )
    ...
    pub async fn record_template_v10_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v10 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_marketplace_templates_v10(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV10Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v10 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v10(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
    ...
    // --- V11: Workflow Template V11 with Reviews V10, Marketplace V11, Analytics V11, Recommendations V11 ---

    pub async fn create_template_v11(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV11, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV11>(
            r#"INSERT INTO workflow_templates_v11 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v11(&self, id: Uuid) -> Result<Option<WorkflowTemplateV11>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV11>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v11 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v11(&self) -> Result<Vec<WorkflowTemplateV11>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV11>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v11 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v11(&self) -> Result<Vec<WorkflowTemplateV11>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV11>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v11 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v11_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV11>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV11>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v11 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v11(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV11>(
            r#"UPDATE workflow_templates_v11 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v11(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v11 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v11_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV10, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV10>(
            r#"INSERT INTO workflow_template_reviews_v10 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v11_rating(template_id).await?;

        Ok(row)
    }

    async fn recalculate_template_v11_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v10 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v11 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v11_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV10, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV10>(
            r#"UPDATE workflow_template_reviews_v10 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v11_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV10>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateReviewV10>(
            r#"SELECT id, template_id, user_id, rating, review, helpful_count, created_at
             FROM workflow_template_reviews_v10 WHERE template_id = $1
             ORDER BY helpful_count DESC, created_at DESC"#,
        )
    ...
    pub async fn record_template_v11_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v11 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v11_analytics(
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v10 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v11 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_marketplace_templates_v11(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV11>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v11 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v11(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
...
// --- V15: Workflow Template V12 with Reviews V11, Marketplace V12, Analytics V12, Recommendations V12 ---

#[derive(Debug, sqlx::FromRow)]
struct WorkflowTemplateV12Row {
    id: Uuid,
    name: String,
    description: String,
    template_type: String,
...
impl From<WorkflowTemplateV12Row> for WorkflowTemplateV12 {
    fn from(row: WorkflowTemplateV12Row) -> Self {
        WorkflowTemplateV12 {
            id: row.id,
            name: row.name,
            description: row.description,
...
#[derive(Debug, sqlx::FromRow)]
struct WorkflowTemplateReviewV11Row {
    id: Uuid,
    template_id: Uuid,
    user_id: Uuid,
    rating: i32,
...
impl From<WorkflowTemplateReviewV11Row> for WorkflowTemplateReviewV11 {
    fn from(row: WorkflowTemplateReviewV11Row) -> Self {
        WorkflowTemplateReviewV11 {
            id: row.id,
            template_id: row.template_id,
            user_id: row.user_id,
            rating: row.rating,
...
impl WorkflowService {
    pub async fn create_template_v12(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV12, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV12Row>(
            r#"INSERT INTO workflow_templates_v12 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v12(&self, id: Uuid) -> Result<Option<WorkflowTemplateV12>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV12Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v12 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v12(&self) -> Result<Vec<WorkflowTemplateV12>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV12Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v12 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v12(&self) -> Result<Vec<WorkflowTemplateV12>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV12Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v12 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v12_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV12>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV12Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v12 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v12(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV12Row>(
            r#"UPDATE workflow_templates_v12 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v12(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v12 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v12_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV11, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV11Row>(
            r#"INSERT INTO workflow_template_reviews_v11 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v12_rating(template_id).await?;

        Ok(row.into())
    }

    async fn recalculate_template_v12_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v11 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v12 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v12_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV11, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV11Row>(
            r#"UPDATE workflow_template_reviews_v11 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v12_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV11>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateReviewV11Row>(
            r#"SELECT id, template_id, user_id, rating, review, helpful_count, created_at
             FROM workflow_template_reviews_v11 WHERE template_id = $1
             ORDER BY helpful_count DESC, created_at DESC"#,
        )
    ...
    pub async fn record_template_v12_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v12 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v12_analytics(
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v11 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v12 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_marketplace_templates_v12(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV12Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v12 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v12(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
...
// --- V16: Workflow Template V13 with Reviews V12, Marketplace V13, Analytics V13, Recommendations V13 ---

#[derive(Debug, sqlx::FromRow)]
struct WorkflowTemplateV13Row {
    id: Uuid,
    name: String,
    description: String,
    template_type: String,
...
impl From<WorkflowTemplateV13Row> for WorkflowTemplateV13 {
    fn from(row: WorkflowTemplateV13Row) -> Self {
        WorkflowTemplateV13 {
            id: row.id,
            name: row.name,
            description: row.description,
...
#[derive(Debug, sqlx::FromRow)]
struct WorkflowTemplateReviewV12Row {
    id: Uuid,
    template_id: Uuid,
    user_id: Uuid,
    rating: i32,
...
impl From<WorkflowTemplateReviewV12Row> for WorkflowTemplateReviewV12 {
    fn from(row: WorkflowTemplateReviewV12Row) -> Self {
        WorkflowTemplateReviewV12 {
            id: row.id,
            template_id: row.template_id,
            user_id: row.user_id,
            rating: row.rating,
...
impl WorkflowService {
    pub async fn create_template_v13(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV13, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV13Row>(
            r#"INSERT INTO workflow_templates_v13 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v13(&self, id: Uuid) -> Result<Option<WorkflowTemplateV13>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV13Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v13 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v13(&self) -> Result<Vec<WorkflowTemplateV13>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV13Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v13 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v13(&self) -> Result<Vec<WorkflowTemplateV13>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV13Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v13 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v13_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV13>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV13Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v13 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v13(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV13Row>(
            r#"UPDATE workflow_templates_v13 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v13(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v13 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v13_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV12, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV12Row>(
            r#"INSERT INTO workflow_template_reviews_v12 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v13_rating(template_id).await?;

        Ok(row.into())
    }

    async fn recalculate_template_v13_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v12 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v13 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v13_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV12, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV12Row>(
            r#"UPDATE workflow_template_reviews_v12 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v13_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV12>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateReviewV12Row>(
            r#"SELECT id, template_id, user_id, rating, review, helpful_count, created_at
             FROM workflow_template_reviews_v12 WHERE template_id = $1
             ORDER BY helpful_count DESC, created_at DESC"#,
        )
    ...
    pub async fn record_template_v13_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v13 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v13_analytics(
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v12 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v13 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_marketplace_templates_v13(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV13Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v13 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v13(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
...
// --- V17: Workflow Template V14 with Reviews V13, Marketplace V14, Analytics V14, Recommendations V14 ---

#[derive(Debug, sqlx::FromRow)]
struct WorkflowTemplateV14Row {
    id: Uuid,
    name: String,
    description: String,
    template_type: String,
...
impl From<WorkflowTemplateV14Row> for WorkflowTemplateV14 {
    fn from(row: WorkflowTemplateV14Row) -> Self {
        WorkflowTemplateV14 {
            id: row.id,
            name: row.name,
            description: row.description,
...
#[derive(Debug, sqlx::FromRow)]
struct WorkflowTemplateReviewV13Row {
    id: Uuid,
    template_id: Uuid,
    user_id: Uuid,
    rating: i32,
...
impl From<WorkflowTemplateReviewV13Row> for WorkflowTemplateReviewV13 {
    fn from(row: WorkflowTemplateReviewV13Row) -> Self {
        WorkflowTemplateReviewV13 {
            id: row.id,
            template_id: row.template_id,
            user_id: row.user_id,
            rating: row.rating,
...
impl WorkflowService {
    pub async fn create_template_v14(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV14, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV14Row>(
            r#"INSERT INTO workflow_templates_v14 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v14(&self, id: Uuid) -> Result<Option<WorkflowTemplateV14>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV14Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v14 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v14(&self) -> Result<Vec<WorkflowTemplateV14>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV14Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v14 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v14(&self) -> Result<Vec<WorkflowTemplateV14>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV14Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v14 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v14_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV14>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV14Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v14 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v14(
        &self,
        id: Uuid,
        input: UpdateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV14, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV14Row>(
            r#"UPDATE workflow_templates_v14 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v14(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v14 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v14_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV13, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV13Row>(
            r#"INSERT INTO workflow_template_reviews_v13 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v14_rating(template_id).await?;

        Ok(row.into())
    }

    async fn recalculate_template_v14_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v13 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v14 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v14_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV13, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV13Row>(
            r#"UPDATE workflow_template_reviews_v13 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v14_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV13>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateReviewV13Row>(
            r#"SELECT id, template_id, user_id, rating, review, helpful_count, created_at
             FROM workflow_template_reviews_v13 WHERE template_id = $1
             ORDER BY helpful_count DESC, created_at DESC"#,
        )
    ...
    pub async fn record_template_v14_usage(
        &self,
        template_id: Uuid,
        _user_id: Uuid,
    ) -> Result<(), sqlx::Error> {
        sqlx::query("UPDATE workflow_templates_v14 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v14_analytics(
        &self,
        template_id: Uuid,
    ) -> Result<WorkflowTemplateAnalytics, sqlx::Error> {
        #[derive(sqlx::FromRow)]
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v13 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v14 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_template_v14_recommendations(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateRecommendation>, sqlx::Error> {
        let template = self.get_template_v14(template_id).await?
            .ok_or_else(|| sqlx::Error::RowNotFound)?;

        let analytics = self.get_template_v14_analytics(template_id).await?;
        let mut recommendations = Vec::new();
    ...
    pub async fn get_marketplace_templates_v14(
        &self,
        limit: i64,
    ) -> Result<Vec<WorkflowTemplateV14>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV14Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v14 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn search_marketplace_templates_v14(
        &self,
        query: &str,
        limit: i64,
    ) -> Result<Vec<WorkflowTemplateV14>, sqlx::Error> {
    ...
    pub async fn create_workflow_from_template_v14(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
        ...
        let _ = self.record_template_v14_usage(template_id, user_id).await;

        Ok(workflow)
    }

    // --- V15: Workflow Template V15 with Reviews V14, Marketplace V15, Analytics V15, Recommendations V15 ---

    pub async fn create_template_v15(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV15, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct WorkflowTemplateV15Row {
            id: Uuid,
            name: String,
            description: String,
            template_type: String,
        ...
        impl From<WorkflowTemplateV15Row> for WorkflowTemplateV15 {
            fn from(row: WorkflowTemplateV15Row) -> Self {
                WorkflowTemplateV15 {
                    id: row.id,
                    name: row.name,
                    description: row.description,
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV15Row>(
            r#"INSERT INTO workflow_templates_v15 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v15(&self, id: Uuid) -> Result<Option<WorkflowTemplateV15>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct WorkflowTemplateV15Row {
            id: Uuid,
            name: String,
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV15Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v15 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v15(&self) -> Result<Vec<WorkflowTemplateV15>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct WorkflowTemplateV15Row {
            id: Uuid,
            name: String,
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV15Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v15 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v15(&self) -> Result<Vec<WorkflowTemplateV15>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct WorkflowTemplateV15Row {
            id: Uuid,
            name: String,
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV15Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v15 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v15_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV15>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct WorkflowTemplateV15Row {
            id: Uuid,
            name: String,
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV15Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v15 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v15(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV15Row>(
            r#"UPDATE workflow_templates_v15 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v15(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v15 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v15_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV14, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct WorkflowTemplateReviewV14Row {
            id: Uuid,
            template_id: Uuid,
            user_id: Uuid,
            rating: i32,
        ...
        impl From<WorkflowTemplateReviewV14Row> for WorkflowTemplateReviewV14 {
            fn from(row: WorkflowTemplateReviewV14Row) -> Self {
                WorkflowTemplateReviewV14 {
                    id: row.id,
                    template_id: row.template_id,
                    user_id: row.user_id,
                    rating: row.rating,
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV14Row>(
            r#"INSERT INTO workflow_template_reviews_v14 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v15_rating(template_id).await?;

        Ok(row.into())
    }

    async fn recalculate_template_v15_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v14 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v15 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v15_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV14, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV14Row>(
            r#"UPDATE workflow_template_reviews_v14 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v15_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV14>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct WorkflowTemplateReviewV14Row {
            id: Uuid,
            template_id: Uuid,
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateReviewV14Row>(
            r#"SELECT id, template_id, user_id, rating, review, helpful_count, created_at
             FROM workflow_template_reviews_v14 WHERE template_id = $1
             ORDER BY helpful_count DESC, created_at DESC"#,
        )
    ...
    pub async fn record_template_v15_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v15 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v15_analytics(
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v14 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v15 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_marketplace_templates_v15(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV15Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v15 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v15(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
    ...
    // --- V16: Workflow Template V16 with Reviews V15, Marketplace V16, Analytics V16, Recommendations V16 ---

    pub async fn create_template_v16(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV16, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct Row {
            id: Uuid,
            name: String,
        ...
        let row = sqlx::query_as::<_, Row>(
            r#"INSERT INTO workflow_templates_v16 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v16(&self, id: Uuid) -> Result<Option<WorkflowTemplateV16>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct Row {
            id: Uuid,
            name: String,
        ...
        let row = sqlx::query_as::<_, Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v16 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;

        Ok(row.map(|r| WorkflowTemplateV16 {
            id: r.id,
            name: r.name,
    ...
    pub async fn list_templates_v16(&self) -> Result<Vec<WorkflowTemplateV16>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct Row {
            id: Uuid,
            name: String,
        ...
        let rows = sqlx::query_as::<_, Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v16 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;

        Ok(rows.into_iter().map(|r| WorkflowTemplateV16 {
            id: r.id,
            name: r.name,
            description: r.description,
            template_type: r.template_type,
    ...
    pub async fn list_public_templates_v16(&self) -> Result<Vec<WorkflowTemplateV16>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct Row {
            id: Uuid,
            name: String,
        ...
        let rows = sqlx::query_as::<_, Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v16 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v16_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV16>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct Row {
            id: Uuid,
            name: String,
        ...
        let rows = sqlx::query_as::<_, Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v16 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v16(
        ...
        let row = sqlx::query_as::<_, Row>(
            r#"UPDATE workflow_templates_v16 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v16(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v16 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v16_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV15, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct Row {
            id: Uuid,
            template_id: Uuid,
        ...
        let row = sqlx::query_as::<_, Row>(
            r#"INSERT INTO workflow_template_reviews_v15 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v16_rating(template_id).await?;

        Ok(WorkflowTemplateReviewV15 {
            id: row.id,
            template_id: row.template_id,
            user_id: row.user_id,
            rating: row.rating,
    ...
    async fn recalculate_template_v16_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v15 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v16 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v16_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV15, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        ...
        let row = sqlx::query_as::<_, Row>(
            r#"UPDATE workflow_template_reviews_v15 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v16_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV15>, sqlx::Error> {
        #[derive(Debug, sqlx::FromRow)]
        struct Row {
            id: Uuid,
            template_id: Uuid,
        ...
        Ok(rows.into_iter().map(|r| WorkflowTemplateReviewV15 {
            id: r.id,
    ...
    pub async fn record_template_v16_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v16 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v16_analytics(
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v15 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v16 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_marketplace_templates_v16(
        ...
        let rows = sqlx::query_as::<_, Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v16 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v16(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
    ...
    // --- V17: Workflow Template V17 with Reviews V16, Marketplace V17, Analytics V17, Recommendations V17 ---

    pub async fn create_template_v17(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV17, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV17>(
            r#"INSERT INTO workflow_templates_v17 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v17(&self, id: Uuid) -> Result<Option<WorkflowTemplateV17>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV17>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v17 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v17(&self) -> Result<Vec<WorkflowTemplateV17>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV17>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v17 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v17(&self) -> Result<Vec<WorkflowTemplateV17>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV17>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v17 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v17_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV17>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV17>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v17 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v17(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV17>(
            r#"UPDATE workflow_templates_v17 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v17(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v17 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v17_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV16, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV16>(
            r#"INSERT INTO workflow_template_reviews_v16 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v17_rating(template_id).await?;

        Ok(row)
    }

    async fn recalculate_template_v17_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v16 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v17 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v17_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV16, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV16>(
            r#"UPDATE workflow_template_reviews_v16 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v17_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV16>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateReviewV16>(
            r#"SELECT id, template_id, user_id, rating, review, helpful_count, created_at
             FROM workflow_template_reviews_v16 WHERE template_id = $1
             ORDER BY helpful_count DESC, created_at DESC"#,
        )
    ...
    pub async fn record_template_v17_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v17 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v17_analytics(
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v16 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v17 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_marketplace_templates_v17(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV17>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v17 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v17(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
    ...
    // --- V18: Workflow Template V18 with Reviews V17, Marketplace V18, Analytics V18, Recommendations V18 ---

    pub async fn create_template_v18(
        &self,
        input: CreateWorkflowTemplateV2,
    ) -> Result<WorkflowTemplateV18, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV18Row>(
            r#"INSERT INTO workflow_templates_v18 (name, description, template_type, config, is_public, author_id)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at"#,
        )
        .bind(&input.name)
    ...
    pub async fn get_template_v18(&self, id: Uuid) -> Result<Option<WorkflowTemplateV18>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateV18Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v18 WHERE id = $1"#,
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v18(&self) -> Result<Vec<WorkflowTemplateV18>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV18Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v18 ORDER BY created_at DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_public_templates_v18(&self) -> Result<Vec<WorkflowTemplateV18>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV18Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v18 WHERE is_public = true ORDER BY rating DESC, usage_count DESC"#,
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn list_templates_v18_by_type(
        &self,
        template_type: &str,
    ) -> Result<Vec<WorkflowTemplateV18>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateV18Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v18 WHERE template_type = $1 AND is_public = true
             ORDER BY rating DESC, usage_count DESC"#,
        )
        .bind(template_type)
        .fetch_all(&self.pool)
    ...
    pub async fn update_template_v18(
        ...
        let row = sqlx::query_as::<_, WorkflowTemplateV18Row>(
            r#"UPDATE workflow_templates_v18 SET
             name = COALESCE($2, name),
             description = COALESCE($3, description),
             template_type = COALESCE($4, template_type),
             config = COALESCE($5, config),
    ...
    pub async fn delete_template_v18(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM workflow_templates_v18 WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn add_template_v18_review(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        rating: i32,
        review: &str,
    ) -> Result<WorkflowTemplateReviewV17, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV17Row>(
            r#"INSERT INTO workflow_template_reviews_v17 (template_id, user_id, rating, review)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (template_id, user_id) DO UPDATE SET rating = $3, review = $4
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        ...
        self.recalculate_template_v18_rating(template_id).await?;

        Ok(row.into())
    }

    async fn recalculate_template_v18_rating(&self, template_id: Uuid) -> Result<(), sqlx::Error> {
        #[derive(sqlx::FromRow)]
        struct RatingAvg {
            avg_rating: Option<f64>,
        }
        ...
        let row = sqlx::query_as::<_, RatingAvg>(
            r#"SELECT AVG(rating::double precision) as avg_rating
             FROM workflow_template_reviews_v17 WHERE template_id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let avg = row.avg_rating.unwrap_or(0.0);
        sqlx::query("UPDATE workflow_templates_v18 SET rating = $2 WHERE id = $1")
            .bind(template_id)
            .bind(avg)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn mark_template_v18_review_helpful(
        &self,
        review_id: Uuid,
    ) -> Result<WorkflowTemplateReviewV17, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkflowTemplateReviewV17Row>(
            r#"UPDATE workflow_template_reviews_v17 SET helpful_count = helpful_count + 1
             WHERE id = $1
             RETURNING id, template_id, user_id, rating, review, helpful_count, created_at"#,
        )
        .bind(review_id)
    ...
    pub async fn get_template_v18_reviews(
        &self,
        template_id: Uuid,
    ) -> Result<Vec<WorkflowTemplateReviewV17>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkflowTemplateReviewV17Row>(
            r#"SELECT id, template_id, user_id, rating, review, helpful_count, created_at
             FROM workflow_template_reviews_v17 WHERE template_id = $1
             ORDER BY helpful_count DESC, created_at DESC"#,
        )
    ...
    pub async fn record_template_v18_usage(
        ...
        sqlx::query("UPDATE workflow_templates_v18 SET usage_count = usage_count + 1 WHERE id = $1")
            .bind(template_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_template_v18_analytics(
        ...
        let row = sqlx::query_as::<_, AnalyticsRow>(
            r#"SELECT
                usage_count as total_usage,
                rating as avg_rating,
                (SELECT COUNT(*) FROM workflow_template_reviews_v17 WHERE template_id = $1) as total_reviews
             FROM workflow_templates_v18 WHERE id = $1"#,
        )
        .bind(template_id)
        .fetch_one(&self.pool)
    ...
    pub async fn get_marketplace_templates_v18(
        ...
        let rows = sqlx::query_as::<_, WorkflowTemplateV18Row>(
            r#"SELECT id, name, description, template_type, config, is_public, author_id, usage_count, rating, created_at
             FROM workflow_templates_v18 WHERE is_public = true
             ORDER BY rating DESC, usage_count DESC LIMIT $1"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    pub async fn create_workflow_from_template_v18(
        &self,
        template_id: Uuid,
        user_id: Uuid,
        workflow_name: Option<&str>,
