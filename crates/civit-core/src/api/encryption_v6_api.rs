...
pub struct KeyVersionResponseV3 {
    pub id: Uuid,
    pub key_id: Uuid,
    pub version: i32,
...
#[derive(Debug, Clone, Serialize)]
pub struct ComplianceCheckResponseV3 {
    pub id: Uuid,
    pub check_type: String,
    pub status: String,
    pub findings: serde_json::Value,
...
#[derive(Debug, Clone, Serialize)]
pub struct KeyLifecycleResponseV3 {
    pub key_id: Uuid,
    pub key_name: String,
...
impl From<KeyVersionV3> for KeyVersionResponseV3 {
    fn from(v: KeyVersionV3) -> Self {
        Self {
            id: v.id,
            key_id: v.key_id,
            version: v.version,
...
impl From<ComplianceCheckV3> for ComplianceCheckResponseV3 {
    fn from(c: ComplianceCheckV3) -> Self {
        Self {
            id: c.id,
...
) -> Result<(StatusCode, Json<KeyVersionResponseV3>), Response> {
    let key_material = input.key_material.as_bytes();
    let version = sqlx::query_as::<_, KeyVersionV3>(
        r#"INSERT INTO encryption_key_versions_v3 (key_id, version, key_material)
         VALUES ($1, $2, $3)
...
) -> Result<Json<Vec<KeyVersionResponseV3>>, Response> {
    let versions = sqlx::query_as::<_, KeyVersionV3>(
        r#"SELECT id, key_id, version, key_material, created_at
         FROM encryption_key_versions_v3
         WHERE key_id = $1
...
) -> Result<(StatusCode, Json<ComplianceCheckResponseV3>), Response> {
    let check = sqlx::query_as::<_, ComplianceCheckV3>(
        r#"INSERT INTO encryption_compliance_checks_v3 (check_type)
         VALUES ($1)
...
) -> Result<Json<ComplianceCheckResponseV3>, Response> {
    let check = sqlx::query_as::<_, ComplianceCheckV3>(
        r#"UPDATE encryption_compliance_checks_v3
...
async fn get_compliance_checks(
    State(state): State<AppState>,
) -> Result<Json<Vec<ComplianceCheckResponseV3>>, Response> {
    let checks = sqlx::query_as::<_, ComplianceCheckV3>(
        r#"SELECT id, check_type, status, findings, score, created_at
         FROM encryption_compliance_checks_v3
...
async fn get_key_lifecycle(
    State(state): State<AppState>,
    Path(key_id): Path<Uuid>,
) -> Result<Json<KeyLifecycleResponseV3>, Response> {
    #[derive(sqlx::FromRow)]
    struct KeyInfo {
        id: Uuid,
        name: String,
    ...
    let versions_count: (i64,) = sqlx::query_as(
        r#"SELECT COUNT(*) FROM encryption_key_versions_v3 WHERE key_id = $1"#,
    )
    .bind(key_id)
    .fetch_one(state.db.pool())
    .await
    ...
    let compliance_score: (i32,) = sqlx::query_as(
        r#"SELECT COALESCE(AVG(score), 0) FROM encryption_compliance_checks_v3"#,
    )
    .fetch_one(state.db.pool())
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))).into_response())?;
    ...
    let latest_version: Option<(Option<i32>,)> = sqlx::query_as(
        r#"SELECT MAX(version) FROM encryption_key_versions_v3 WHERE key_id = $1"#,
    )
    .bind(key_id)
    .fetch_optional(state.db.pool())
    .await
    ...
    Ok(Json(KeyLifecycleResponseV3 {
        key_id: key.id,
        key_name: key.name,
...
pub fn encryption_v6_routes() -> Router<AppState> {
    Router::new()
        .route(
            "/api/v1/encryption-v6/key-versions",
            post(create_key_version),
        )
        .route(
            "/api/v1/encryption-v6/key-versions/{key_id}",
            get(get_key_versions),
        )
        .route(
            "/api/v1/encryption-v6/compliance-checks",
            post(create_compliance_check).get(get_compliance_checks),
        )
        .route(
            "/api/v1/encryption-v6/compliance-checks/{check_id}",
            patch(update_compliance_check),
        )
        .route(
            "/api/v1/encryption-v6/key-lifecycle/{key_id}",
            get(get_key_lifecycle),
        )
        .route(
            "/api/v1/encryption-v6/audit-log",
...
mod tests {
    ...
    fn test_key_version_response_conversion() {
        ...
        let response: KeyVersionResponseV3 = version.into();
        assert_eq!(response.version, 1);
    }

    #[test]
    fn test_compliance_check_response_conversion() {
        let check = ComplianceCheckV3 {
            id: Uuid::nil(),
        ...
        let response: ComplianceCheckResponseV3 = check.into();
