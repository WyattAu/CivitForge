...
pub struct TestSuiteMetricResponseV5 {
    pub id: String,
    pub suite_id: String,
...
#[derive(Debug, Serialize)]
pub struct TestSuiteBaselineResponseV5 {
    pub id: String,
    pub suite_id: String,
    pub metric_name: String,
...
pub struct RecordMetricRequestV5 {
    pub metric_name: String,
    pub metric_value: f64,
}

#[derive(Debug, Deserialize)]
pub struct CreateBaselineRequestV5 {
    pub metric_name: String,
    pub baseline_value: f64,
...
#[derive(Debug, Deserialize)]
pub struct RegressionCheckRequestV5 {
    pub metric_name: String,
    pub current_value: f64,
}

#[derive(Debug, Serialize)]
pub struct RegressionCheckResponseV5 {
...
#[derive(Debug, Deserialize)]
pub struct ListMetricsParamsV5 {
...
pub async fn record_metric_v5(
    ...
    _auth: AuthUser,
    Json(req): Json<RecordMetricRequestV5>,
) -> impl IntoResponse {
    match state
        .db
        .create_test_suite_metric_v5(suite_id, &req.metric_name, req.metric_value)
        .await
    {
        Ok(metric) => (
            StatusCode::CREATED,
            Json(TestSuiteMetricResponseV5 {
...
pub async fn list_metrics_v5(
    ...
    Path(suite_id): Path<Uuid>,
    _auth: AuthUser,
    Query(params): Query<ListMetricsParamsV5>,
) -> impl IntoResponse {
    match state
        .db
        .list_test_suite_metrics_v5(suite_id, params.limit, params.offset)
        .await
    {
        Ok(metrics) => {
            let resp: Vec<TestSuiteMetricResponseV5> = metrics
                .into_iter()
                .map(|m| TestSuiteMetricResponseV5 {
                    id: m.id.to_string(),
...
pub async fn get_latest_metric_v5(
    State(state): State<AppState>,
    ...
    match state
        .db
        .get_test_suite_latest_metric_v5(suite_id, &metric_name)
        .await
    {
        Ok(Some(metric)) => (
            StatusCode::OK,
...
pub async fn create_baseline_v5(
    State(state): State<AppState>,
    Path(suite_id): Path<Uuid>,
    _auth: AuthUser,
    Json(req): Json<CreateBaselineRequestV5>,
) -> impl IntoResponse {
    let threshold = req.threshold_percent.unwrap_or(10.0);
    match state
        .db
        .create_test_suite_baseline_v5(suite_id, &req.metric_name, req.baseline_value, threshold)
        .await
    {
        Ok(baseline) => (
            StatusCode::CREATED,
            Json(TestSuiteBaselineResponseV5 {
...
pub async fn list_baselines_v5(
    State(state): State<AppState>,
    Path(suite_id): Path<Uuid>,
    _auth: AuthUser,
) -> impl IntoResponse {
    match state.db.get_test_suite_baselines_v5(suite_id).await {
        Ok(baselines) => {
            let resp: Vec<TestSuiteBaselineResponseV5> = baselines
                .into_iter()
                .map(|b| TestSuiteBaselineResponseV5 {
                    id: b.id.to_string(),
...
pub async fn check_regression_v5(
    State(state): State<AppState>,
    Path(suite_id): Path<Uuid>,
    _auth: AuthUser,
    Json(req): Json<RegressionCheckRequestV5>,
) -> impl IntoResponse {
    match state
        .db
        .detect_test_suite_regression_v5(suite_id, &req.metric_name, req.current_value)
        .await
    {
        Ok(is_regression) => (
            StatusCode::OK,
            Json(RegressionCheckResponseV5 {
                is_regression,
...
pub fn test_suite_v5_routes() -> Router<AppState> {
    Router::new()
        .route(
            "/api/v1/test-suites/{suite_id}/metrics",
            post(record_metric_v5).get(list_metrics_v5),
        )
        .route(
            "/api/v1/test-suites/{suite_id}/metrics/{metric_name}/latest",
            get(get_latest_metric_v5),
        )
        .route(
            "/api/v1/test-suites/{suite_id}/baselines",
            post(create_baseline_v5).get(list_baselines_v5),
        )
        .route(
            "/api/v1/test-suites/{suite_id}/regression-check",
            post(check_regression_v5),
        )
}
...
mod tests {
    ...
    fn test_metric_response_serializes() {
        let resp = TestSuiteMetricResponseV5 {
    ...
    fn test_baseline_response_serializes() {
        let resp = TestSuiteBaselineResponseV5 {
            id: "00000000-0000-0000-0000-000000000001".into(),
            suite_id: "00000000-0000-0000-0000-000000000002".into(),
    ...
    fn test_regression_check_response() {
        let resp = RegressionCheckResponseV5 {
            is_regression: true,
    ...
    fn test_record_metric_request() {
        let req: RecordMetricRequestV5 = serde_json::from_str(
    ...
    fn test_create_baseline_request() {
        let req: CreateBaselineRequestV5 = serde_json::from_str(
