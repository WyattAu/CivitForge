...
pub struct CodeQualityMetricResponseV20 {
    pub id: String,
    pub repo_id: String,
...
#[derive(Debug, Serialize)]
pub struct CodeQualityThresholdResponseV20 {
    pub id: String,
    pub repo_id: String,
    pub metric_name: String,
...
#[derive(Debug, Deserialize)]
pub struct CreateThresholdRequestV20 {
    pub metric_name: String,
    pub threshold_value: f64,
...
#[derive(Debug, Deserialize)]
pub struct CheckViolationRequestV20 {
    pub metric_name: String,
    pub metric_value: f64,
}

#[derive(Debug, Serialize)]
pub struct CheckViolationResponseV20 {
...
#[derive(Debug, Serialize)]
pub struct QualityScoreResponseV20 {
    pub repo_id: String,
    pub score: f64,
}

#[derive(Debug, Serialize)]
pub struct EnforcementReportResponseV20 {
...
#[derive(Debug, Deserialize)]
pub struct ListThresholdsParamsV20 {
    #[serde(default)]
...
pub async fn record_metric_v20(
        ...
        .await
    {
        Ok(metric) => (
            StatusCode::CREATED,
            Json(CodeQualityMetricResponseV20 {
...
pub async fn list_metrics_v20(
    ...
    {
        Ok(metrics) => {
            let resp: Vec<CodeQualityMetricResponseV20> = metrics
                .into_iter()
                .map(|m| CodeQualityMetricResponseV20 {
                    id: m.id.to_string(),
...
pub async fn get_quality_score_v20(
    State(state): State<AppState>,
    Path(repo_id): Path<Uuid>,
    _auth: AuthUser,
) -> impl IntoResponse {
...
pub async fn create_threshold_v20(
    State(state): State<AppState>,
    Path(repo_id): Path<Uuid>,
    _auth: AuthUser,
    Json(req): Json<CreateThresholdRequestV20>,
) -> impl IntoResponse {
        ...
        .await
    {
        Ok(threshold) => (
            StatusCode::CREATED,
            Json(CodeQualityThresholdResponseV20 {
...
pub async fn list_thresholds_v20(
    State(state): State<AppState>,
    Path(repo_id): Path<Uuid>,
    _auth: AuthUser,
    Query(params): Query<ListThresholdsParamsV20>,
) -> impl IntoResponse {
    match state
        .db
    ...
    {
        Ok(thresholds) => {
            let resp: Vec<CodeQualityThresholdResponseV20> = thresholds
                .into_iter()
                .map(|t| CodeQualityThresholdResponseV20 {
                    id: t.id.to_string(),
...
pub async fn check_violation_v20(
    State(state): State<AppState>,
    Path(repo_id): Path<Uuid>,
    _auth: AuthUser,
    Json(req): Json<CheckViolationRequestV20>,
) -> impl IntoResponse {
    match state
        .db
        ...
        .await
    {
        Ok(is_violation) => (
            StatusCode::OK,
            Json(CheckViolationResponseV20 {
                is_violation,
...
pub async fn delete_threshold_v20(
    State(state): State<AppState>,
...
pub async fn get_enforcement_report_v20(
    State(state): State<AppState>,
    Path(repo_id): Path<Uuid>,
    _auth: AuthUser,
) -> impl IntoResponse {
    match state.db.get_code_quality_enforcement_report_v20(repo_id).await {
        Ok((total, violating, compliance_rate)) => (
            StatusCode::OK,
            Json(EnforcementReportResponseV20 {
...
pub fn code_quality_v20_routes() -> Router<AppState> {
    Router::new()
        .route(
            "/api/v1/repos/{repo_id}/quality/metrics/v20",
            post(record_metric_v20).get(list_metrics_v20),
        )
        .route(
            "/api/v1/repos/{repo_id}/quality/score/v20",
            get(get_quality_score_v20),
        )
        .route(
            "/api/v1/repos/{repo_id}/quality/thresholds/v20",
            post(create_threshold_v20).get(list_thresholds_v20),
        )
        .route(
            "/api/v1/repos/{repo_id}/quality/check-violation/v20",
            post(check_violation_v20),
        )
        .route(
            "/api/v1/repos/{repo_id}/quality/thresholds/{metric_name}/v20",
            delete(delete_threshold_v20),
        )
        .route(
            "/api/v1/repos/{repo_id}/quality/enforcement-report/v20",
            get(get_enforcement_report_v20),
        )
}
...
mod tests {
    ...
    fn test_metric_response_v20_serializes() {
        let resp = CodeQualityMetricResponseV20 {
    ...
    #[test]
    fn test_threshold_response_v20_serializes() {
        let resp = CodeQualityThresholdResponseV20 {
            id: "00000000-0000-0000-0000-000000000001".into(),
            repo_id: "00000000-0000-0000-0000-000000000002".into(),
    ...
    #[test]
    fn test_violation_check_v20_response() {
        let resp = CheckViolationResponseV20 {
            is_violation: false,
    ...
    #[test]
    fn test_quality_score_v20_response() {
        let resp = QualityScoreResponseV20 {
    ...
    #[test]
    fn test_enforcement_report_v20_response() {
        let resp = EnforcementReportResponseV20 {
            total_thresholds: 10,
    ...
    #[test]
    fn test_create_threshold_v20_request() {
        let req: CreateThresholdRequestV20 = serde_json::from_str(
