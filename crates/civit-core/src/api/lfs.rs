...
async fn ensure_lfs_table(pool: &sqlx::postgres::PgPool) {
    let _ = sqlx::query(
...
pub async fn put_object(
    ...
    let pool = state.db.pool();
    let _ = ensure_lfs_table(pool).await;
...
mod tests {
    ...
    #[test]
    fn test_lfs_batch_request_multiple_objects() {
        let json = r#"{
            "operation": "download",
            "objects": [
                {"oid": "aaa", "size": 100},
    ...
    #[test]
    fn test_lfs_error_response_serialization() {
        let resp = LfsObjectResponse {
            oid: "abc".into(),
            size: 100,
            actions: None,
            error: Some(LfsError {
    ...
    #[test]
    fn test_lfs_verify_response_serialization() {
    ...
    #[test]
    fn test_lfs_verify_error_response() {
        let resp = LfsVerifyResponse {
            oid: "abc".into(),
            size: 0,
    ...
    #[test]
    fn test_lfs_object_ref_parse() {
        let json = r#"{"oid":"abc123","size":512}"#;
        let obj: LfsObjectRef = serde_json::from_str(json).unwrap();
