...
pub fn replication_v9_routes() -> Router<AppState> {
    Router::new()
        .route(
            "/api/v1/replication-v9/config",
            post(set_config),
        )
        .route(
            "/api/v1/replication-v9/config/{replica_id}",
            get(get_all_config),
        )
        .route(
            "/api/v1/replication-v9/config/{replica_id}/{config_key}",
            get(get_config),
        )
        .route(
            "/api/v1/replication-v9/alerts",
            post(create_alert),
        )
        .route(
            "/api/v1/replication-v9/alerts/{replica_id}",
            get(get_alerts),
        )
        .route(
            "/api/v1/replication-v9/monitor",
            get(monitor_replication),
        )
        .route(
            "/api/v1/replication-v9/failover-test/{replica_id}",
            post(test_failover),
        )
}
