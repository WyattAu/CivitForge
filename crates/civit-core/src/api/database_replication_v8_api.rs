...
pub fn replication_v8_routes() -> Router<AppState> {
    Router::new()
        .route(
            "/api/v1/replication-v8/config",
            post(set_config),
        )
        .route(
            "/api/v1/replication-v8/config/{replica_id}",
            get(get_all_config),
        )
        .route(
            "/api/v1/replication-v8/config/{replica_id}/{config_key}",
            get(get_config),
        )
        .route(
            "/api/v1/replication-v8/alerts",
            post(create_alert),
        )
        .route(
            "/api/v1/replication-v8/alerts/{replica_id}",
            get(get_alerts),
        )
        .route(
            "/api/v1/replication-v8/monitor",
            get(monitor_replication),
        )
        .route(
            "/api/v1/replication-v8/failover-test/{replica_id}",
            post(test_failover),
        )
}
