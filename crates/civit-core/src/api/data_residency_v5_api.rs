...
#[derive(Debug, Clone, Serialize, Deserialize, sqlx::FromRow)]
pub struct ResidencyComplianceV3 {
    pub id: Uuid,
    pub rule_id: Uuid,
...
pub struct ResidencyReportResponseV3 {
    pub id: Uuid,
    pub report_type: String,
    pub findings: serde_json::Value,
    pub score: i32,
...
#[derive(Debug, Clone, Serialize)]
pub struct ResidencyComplianceResponseV3 {
    pub id: Uuid,
    pub rule_id: Uuid,
    pub compliance_status: String,
...
impl From<ResidencyReportV3> for ResidencyReportResponseV3 {
    fn from(r: ResidencyReportV3) -> Self {
        Self {
            id: r.id,
...
impl From<ResidencyComplianceV3> for ResidencyComplianceResponseV3 {
    fn from(c: ResidencyComplianceV3) -> Self {
        Self {
            id: c.id,
            rule_id: c.rule_id,
...
) -> Result<(StatusCode, Json<ResidencyReportResponseV3>), Response> {
    let report = sqlx::query_as::<_, ResidencyReportV3>(
        r#"INSERT INTO data_residency_reports_v3 (report_type)
         VALUES ($1)
...
async fn get_reports(
    State(state): State<AppState>,
) -> Result<Json<Vec<ResidencyReportResponseV3>>, Response> {
    let reports = sqlx::query_as::<_, ResidencyReportV3>(
        r#"SELECT id, report_type, findings, score, generated_at
         FROM data_residency_reports_v3
...
) -> Result<(StatusCode, Json<ResidencyComplianceResponseV3>), Response> {
    let compliance = sqlx::query_as::<_, ResidencyComplianceV3>(
        r#"INSERT INTO data_residency_compliance_v3 (rule_id)
         VALUES ($1)
...
) -> Result<Json<ResidencyComplianceResponseV3>, Response> {
    let compliance = sqlx::query_as::<_, ResidencyComplianceV3>(
        r#"UPDATE data_residency_compliance_v3
...
) -> Result<Json<Vec<ResidencyComplianceResponseV3>>, Response> {
    let entries = sqlx::query_as::<_, ResidencyComplianceV3>(
        r#"SELECT id, rule_id, compliance_status, last_checked_at, created_at
         FROM data_residency_compliance_v3
         WHERE rule_id = $1
...
async fn get_residency_compliance_summary(
    ...
    let average_score = sqlx::query_scalar::<_, f64>(
        r#"SELECT COALESCE(AVG(score), 0.0) FROM data_residency_reports_v3"#,
    )
    .fetch_one(state.db.pool())
    .await
    .unwrap_or(0.0);
...
pub fn data_residency_v5_routes() -> Router<AppState> {
    Router::new()
        .route(
            "/api/v1/data-residency-v5/reports",
            post(generate_report).get(get_reports),
        )
        .route(
            "/api/v1/data-residency-v5/compliance",
            post(track_compliance).get(get_residency_compliance_summary),
        )
        .route(
            "/api/v1/data-residency-v5/compliance/{compliance_id}",
            patch(update_compliance_status),
        )
        .route(
            "/api/v1/data-residency-v5/compliance/rule/{rule_id}",
            get(get_compliance_by_rule),
        )
        .route(
            "/api/v1/data-residency-v5/violations/{violation_id}/resolve",
...
mod tests {
    ...
    fn test_report_response_conversion() {
        ...
        let response: ResidencyReportResponseV3 = report.into();
    ...
    fn test_compliance_response_conversion() {
        let compliance = ResidencyComplianceV3 {
            id: Uuid::nil(),
            rule_id: Uuid::nil(),
        ...
        let response: ResidencyComplianceResponseV3 = compliance.into();
