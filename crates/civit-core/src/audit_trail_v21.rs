...
impl Default for GeoLocationV21 {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct AuditTrailEventV21 {
    ...
    pub geo_location: Option<GeoLocationV21>,
    pub risk_score: u32,
    pub compliance_status: ComplianceStatusV21,
    pub created_at: DateTime<Utc>,
}

impl AuditTrailEventV21 {
    pub fn new(
        event_type: &str,
        resource_type: &str,
        resource_id: &str,
        ...
        Self {
            ...
            geo_location: None,
            risk_score: 0,
            compliance_status: ComplianceStatusV21::Unknown,
            created_at: Utc::now(),
        }
    }
    ...
    pub fn with_compliance_status(mut self, status: ComplianceStatusV21) -> Self {
    ...
    pub fn is_compliant(&self) -> bool {
        self.compliance_status == ComplianceStatusV21::Compliant
    }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditTrailQueryV21 {
    ...
    pub request_id: Option<String>,
    pub session_id: Option<String>,
    pub compliance_status: Option<ComplianceStatusV21>,
...
impl Default for AuditTrailQueryV21 {
    fn default() -> Self {
        Self {
            event_type: None,
            resource_type: None,
...
impl AuditTrailQueryV21 {
    pub fn matches(&self, event: &AuditTrailEventV21) -> bool {
...
pub struct AuditTrailRecorderV21 {
    events: std::sync::Mutex<Vec<AuditTrailEventV21>>,
}

impl AuditTrailRecorderV21 {
    pub fn new() -> Self {
        Self {
    ...
    pub fn record(&self, event: AuditTrailEventV21) {
        let mut events = self.events.lock().unwrap();
        events.push(event);
    }

    pub fn search(&self, query: &AuditTrailQueryV21) -> Vec<AuditTrailEventV21> {
    ...
    pub fn get_session_events(&self, session_id: &str) -> Vec<AuditTrailEventV21> {
        let events = self.events.lock().unwrap();
        events
            .iter()
            .filter(|e| e.session_id.as_deref() == Some(session_id))
    ...
    pub fn get_request_events(&self, request_id: &str) -> Vec<AuditTrailEventV21> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn get_events_by_geo_country(&self, country: &str) -> Vec<AuditTrailEventV21> {
        let events = self.events.lock().unwrap();
        events
            .iter()
            .filter(|e| {
    ...
    pub fn get_events_by_compliance_status(&self, status: ComplianceStatusV21) -> Vec<AuditTrailEventV21> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn get_high_risk_events(&self, threshold: u32) -> Vec<AuditTrailEventV21> {
        let events = self.events.lock().unwrap();
        events
            .iter()
    ...
    pub fn compliance_audit(
        ...
        let compliant_count = relevant
            .iter()
            .filter(|e| e.compliance_status == ComplianceStatusV21::Compliant)
            .count() as u32;
    ...
    pub fn detect_anomalies(&self) -> Vec<AnomalyDetectionResultV21> {
        let events = self.events.lock().unwrap();
        ...
        for (actor, count) in &actor_event_counts {
            if *count as f64 > avg_events_per_actor * 2.0 && *count > 10 {
                anomalies.push(AnomalyDetectionResultV21 {
                    anomaly_type: AnomalyTypeV21::HighActivity,
        ...
        for event in events.iter() {
            if event.compliance_status == ComplianceStatusV21::NonCompliant {
                failed_count += 1;
            }
            total_count += 1;
        }
        ...
        if total_count > 10 && (failed_count as f64 / total_count as f64) > 0.3 {
            anomalies.push(AnomalyDetectionResultV21 {
                anomaly_type: AnomalyTypeV21::ComplianceDrift,
                actor_id: None,
                description: format!(
        ...
        for (actor, ips) in &actor_ips {
            if ips.len() > 3 {
                anomalies.push(AnomalyDetectionResultV21 {
                    anomaly_type: AnomalyTypeV21::UnusualAccess,
                    actor_id: Some(actor.clone()),
                    description: format!(
    ...
    pub fn risk_assessment(&self) -> RiskAssessmentResultV21 {
        let events = self.events.lock().unwrap();
    ...
    pub fn export(&self, query: &AuditTrailQueryV21) -> AuditExportV21 {
        let events = self.search(query);
        AuditExportV21 {
            format: ExportFormatV21::Json,
...
impl Default for AuditTrailRecorderV21 {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceAuditResultV21 {
    pub event_type: String,
...
pub struct ForensicsExportV21 {
    ...
    pub events: Vec<AuditTrailEventV21>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditExportV21 {
    pub format: ExportFormatV21,
...
#[serde(rename_all = "snake_case")]
pub enum ExportFormatV21 {
    Json,
    Csv,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuditTrailBuilderV21 {
    event_type: String,
    ...
    geo_location: Option<GeoLocationV21>,
    risk_score: u32,
    compliance_status: ComplianceStatusV21,
}

impl AuditTrailBuilderV21 {
    ...
    pub fn geo_location(mut self, geo: GeoLocationV21) -> Self {
    ...
    pub fn compliance_status(mut self, status: ComplianceStatusV21) -> Self {
        self.compliance_status = status;
        self
    }

    pub fn build(self) -> AuditTrailEventV21 {
        AuditTrailEventV21 {
            id: uuid::Uuid::new_v4().to_string(),
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AnomalyDetectionResultV21 {
    pub anomaly_type: AnomalyTypeV21,
    pub actor_id: Option<String>,
    pub description: String,
    pub risk_level: RiskLevelV21,
    pub detected_at: DateTime<Utc>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RiskAssessmentResultV21 {
    pub total_events: u32,
...
pub fn record_compliance_event_v21(
    recorder: &AuditTrailRecorderV21,
    repo_id: &str,
    action: &str,
    framework_name: &str,
    details: serde_json::Value,
    ...
    let event = AuditTrailBuilderV21::new("compliance", "repository", repo_id, action)
...
pub fn record_security_event_v21(
    recorder: &AuditTrailRecorderV21,
    repo_id: &str,
    action: &str,
    scan_type: &str,
    ...
    let event = AuditTrailBuilderV21::new("security", "repository", repo_id, action)
        .details(serde_json::json!({
            "scan_type": scan_type,
            "details": details,
        }))
...
mod tests {
    ...
    fn make_event(event_type: &str, resource_type: &str, resource_id: &str, action: &str) -> AuditTrailEventV21 {
        AuditTrailEventV21 {
            id: uuid::Uuid::new_v4().to_string(),
            event_type: event_type.into(),
            resource_type: resource_type.into(),
    ...
    #[test]
    fn test_geo_location_v21_new() {
        let geo = GeoLocationV21::new();
    ...
    #[test]
    fn test_geo_location_v21_with_fields() {
        let geo = GeoLocationV21::new()
            .with_country("US")
            .with_region("CA")
    ...
    #[test]
    fn test_audit_trail_event_v21_new() {
        let event = AuditTrailEventV21::new("access", "repo", "r1", "read");
        ...
        assert_eq!(event.compliance_status, ComplianceStatusV21::Unknown);
    }

    #[test]
    fn test_audit_trail_event_v21_with_risk_score() {
        let event = AuditTrailEventV21::new("access", "repo", "r1", "read")
            .with_risk_score(75)
            .with_compliance_status(ComplianceStatusV21::NonCompliant);
        assert_eq!(event.risk_score, 75);
        assert_eq!(event.compliance_status, ComplianceStatusV21::NonCompliant);
    }
    ...
    fn test_event_helpers() {
        let event = AuditTrailEventV21::new("access", "repo", "r1", "read")
            .with_risk_score(80)
            .with_compliance_status(ComplianceStatusV21::Compliant);
    ...
    fn test_compliance_status_risk_contribution() {
        assert_eq!(ComplianceStatusV21::Compliant.risk_contribution(), 0);
        assert_eq!(ComplianceStatusV21::Partial.risk_contribution(), 50);
        assert_eq!(ComplianceStatusV21::NonCompliant.risk_contribution(), 100);
        assert_eq!(ComplianceStatusV21::Unknown.risk_contribution(), 25);
    }

    #[test]
    fn test_compliance_status_display_name() {
        assert_eq!(ComplianceStatusV21::Compliant.display_name(), "Compliant");
        assert_eq!(ComplianceStatusV21::NonCompliant.display_name(), "Non-Compliant");
    }

    #[test]
    fn test_audit_trail_query_v21_matches() {
        let mut event = AuditTrailEventV21::new("access", "repo", "r1", "read");
        event.request_id = Some("req-1".into());
        event.session_id = Some("sess-1".into());
        event.risk_score = 50;
        event.compliance_status = ComplianceStatusV21::Compliant;
        let query = AuditTrailQueryV21 {
            request_id: Some("req-1".into()),
            ..AuditTrailQueryV21::default()
        };
        assert!(query.matches(&event));
        let query2 = AuditTrailQueryV21 {
    ...
    #[test]
    fn test_audit_trail_query_v21_matches_risk_score() {
        let mut event = AuditTrailEventV21::new("access", "repo", "r1", "read");
        event.risk_score = 75;
        let query = AuditTrailQueryV21 {
    ...
    #[test]
    fn test_audit_trail_query_v21_matches_compliance_status() {
        let mut event = AuditTrailEventV21::new("access", "repo", "r1", "read");
        event.compliance_status = ComplianceStatusV21::Compliant;
        let query = AuditTrailQueryV21 {
            compliance_status: Some(ComplianceStatusV21::Compliant),
            ..AuditTrailQueryV21::default()
        };
        assert!(query.matches(&event));
        let query2 = AuditTrailQueryV21 {
            compliance_status: Some(ComplianceStatusV21::NonCompliant),
            ..AuditTrailQueryV21::default()
        };
        assert!(!query2.matches(&event));
    }
    ...
    #[test]
    fn test_audit_trail_recorder_v21_record_and_search() {
        let recorder = AuditTrailRecorderV21::new();
        let event = AuditTrailEventV21::new("access", "repo", "r1", "read");
        recorder.record(event);
        assert_eq!(recorder.count(), 1);
        let results = recorder.search(&AuditTrailQueryV21::default());
        assert_eq!(results.len(), 1);
    }

    #[test]
    fn test_audit_trail_recorder_v21_session_tracking() {
        let recorder = AuditTrailRecorderV21::new();
        let mut e1 = AuditTrailEventV21::new("access", "repo", "r1", "read");
        e1.session_id = Some("sess-1".into());
        let mut e2 = AuditTrailEventV21::new("access", "repo", "r1", "read");
        e2.session_id = Some("sess-1".into());
        let mut e3 = AuditTrailEventV21::new("access", "repo", "r1", "read");
    ...
    #[test]
    fn test_audit_trail_recorder_v21_geo_tracking() {
        let recorder = AuditTrailRecorderV21::new();
        let mut e1 = AuditTrailEventV21::new("access", "repo", "r1", "read");
        e1.geo_location = Some(GeoLocationV21::new().with_country("US"));
        let mut e2 = AuditTrailEventV21::new("access", "repo", "r2", "read");
        e2.geo_location = Some(GeoLocationV21::new().with_country("UK"));
        let mut e3 = AuditTrailEventV21::new("access", "repo", "r3", "read");
        e3.geo_location = Some(GeoLocationV21::new().with_country("US"));
        recorder.record(e1);
        recorder.record(e2);
        recorder.record(e3);
    ...
    #[test]
    fn test_audit_trail_recorder_v21_compliance_tracking() {
        let recorder = AuditTrailRecorderV21::new();
        let mut e1 = AuditTrailEventV21::new("access", "repo", "r1", "read");
        e1.compliance_status = ComplianceStatusV21::Compliant;
        let mut e2 = AuditTrailEventV21::new("access", "repo", "r2", "read");
        e2.compliance_status = ComplianceStatusV21::NonCompliant;
        let mut e3 = AuditTrailEventV21::new("access", "repo", "r3", "read");
        e3.compliance_status = ComplianceStatusV21::Compliant;
        recorder.record(e1);
        recorder.record(e2);
        recorder.record(e3);
        assert_eq!(recorder.get_events_by_compliance_status(ComplianceStatusV21::Compliant).len(), 2);
        assert_eq!(recorder.get_events_by_compliance_status(ComplianceStatusV21::NonCompliant).len(), 1);
    }

    #[test]
    fn test_audit_trail_recorder_v21_high_risk_events() {
        let recorder = AuditTrailRecorderV21::new();
        let mut e1 = AuditTrailEventV21::new("access", "repo", "r1", "read");
        e1.risk_score = 90;
        let mut e2 = AuditTrailEventV21::new("access", "repo", "r2", "read");
    ...
    #[test]
    fn test_audit_trail_recorder_v21_compliance_audit() {
        let recorder = AuditTrailRecorderV21::new();
        let mut e1 = AuditTrailEventV21::new("security", "repo", "r1", "scan");
        e1.actor_id = Some("user-1".into());
        e1.risk_score = 75;
        e1.compliance_status = ComplianceStatusV21::Compliant;
        let mut e2 = AuditTrailEventV21::new("security", "repo", "r2", "scan");
        e2.actor_id = Some("user-1".into());
        e2.risk_score = 25;
        e2.compliance_status = ComplianceStatusV21::NonCompliant;
        let mut e3 = AuditTrailEventV21::new("security", "repo", "r1", "fix");
    ...
    #[test]
    fn test_audit_trail_recorder_v21_forensics_export() {
        let recorder = AuditTrailRecorderV21::new();
        let mut e1 = AuditTrailEventV21::new("access", "repo", "r1", "read");
        e1.actor_id = Some("user-1".into());
        e1.session_id = Some("sess-1".into());
        ...
        e1.risk_score = 50;
        e1.compliance_status = ComplianceStatusV21::Compliant;
        let mut e2 = AuditTrailEventV21::new("access", "repo", "r2", "write");
        e2.actor_id = Some("user-1".into());
        e2.session_id = Some("sess-1".into());
    ...
    #[test]
    fn test_audit_trail_recorder_v21_detect_anomalies() {
        let recorder = AuditTrailRecorderV21::new();
        for i in 0..20 {
            let mut e = AuditTrailEventV21::new("access", "repo", &format!("r{}", i), "read");
            e.actor_id = Some("user-1".into());
            e.risk_score = 10;
            e.compliance_status = ComplianceStatusV21::Compliant;
            recorder.record(e);
        }
        for i in 0..5 {
            let mut e = AuditTrailEventV21::new("access", "repo", &format!("r{}", i + 20), "read");
        ...
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV21::HighActivity));
    }

    #[test]
    fn test_audit_trail_recorder_v21_detect_anomalies_compliance_drift() {
        let recorder = AuditTrailRecorderV21::new();
        for i in 0..15 {
            let mut e = AuditTrailEventV21::new("security", "repo", &format!("r{}", i), "scan");
            e.actor_id = Some("user-1".into());
            e.compliance_status = ComplianceStatusV21::NonCompliant;
            recorder.record(e);
        }
        let anomalies = recorder.detect_anomalies();
        assert!(!anomalies.is_empty());
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV21::ComplianceDrift));
    }

    #[test]
    fn test_audit_trail_recorder_v21_detect_anomalies_unusual_access() {
        let recorder = AuditTrailRecorderV21::new();
        let mut e1 = AuditTrailEventV21::new("access", "repo", "r1", "read");
        e1.actor_id = Some("user-1".into());
        e1.ip_address = Some("10.0.0.1".into());
        recorder.record(e1);
        for i in 0..5 {
            let mut e = AuditTrailEventV21::new("access", "repo", &format!("r{}", i + 2), "read");
            e.actor_id = Some("user-1".into());
            e.ip_address = Some(format!("10.0.0.{}", i + 2));
            recorder.record(e);
        }
        let anomalies = recorder.detect_anomalies();
        assert!(anomalies.iter().any(|a| a.anomaly_type == AnomalyTypeV21::UnusualAccess));
    }
    ...
    #[test]
    fn test_audit_trail_builder_v21() {
        let geo = GeoLocationV21::new()
            .with_country("US")
            .with_city("New York");
        let event = AuditTrailBuilderV21::new("security", "repo", "r1", "scan")
            .actor_id("user-1")
            .ip_address("10.0.0.1")
            ...
            .geo_location(geo)
            .risk_score(75)
            .compliance_status(ComplianceStatusV21::Compliant)
        ...
        assert_eq!(event.risk_score, 75);
        assert_eq!(event.compliance_status, ComplianceStatusV21::Compliant);
        assert_eq!(event.details["key"], "value");
    }

    #[test]
    fn test_audit_trail_export_v21() {
        let recorder = AuditTrailRecorderV21::new();
        recorder.record(AuditTrailEventV21::new("access", "repo", "r1", "read"));
        let export = recorder.export(&AuditTrailQueryV21::default());
        assert_eq!(export.total_events, 1);
        assert_eq!(export.format, ExportFormatV21::Json);
    }

    #[test]
    fn test_record_compliance_event_v21() {
        let recorder = AuditTrailRecorderV21::new();
        record_compliance_event_v21(
            &recorder,
            "repo-1",
            "assessment_complete",
            "SOC 2",
        ...
        assert_eq!(results[0].compliance_status, ComplianceStatusV21::Compliant);
    }

    #[test]
    fn test_record_security_event_v21() {
        let recorder = AuditTrailRecorderV21::new();
        record_security_event_v21(
            &recorder,
            "repo-1",
            "scan_completed",
            "sast",
    ...
    fn test_risk_level_display_name() {
        assert_eq!(RiskLevelV21::Critical.display_name(), "Critical");
        assert_eq!(RiskLevelV21::Low.display_name(), "Low");
    }

    #[test]
    fn test_export_format_v21_serialization() {
        assert_eq!(
            serde_json::to_string(&ExportFormatV21::Json).unwrap(),
            "\"json\""
        );
        assert_eq!(
            serde_json::to_string(&ExportFormatV21::Csv).unwrap(),
            "\"csv\""
        );
    }

    #[test]
    fn test_anomaly_type_v21_serialization() {
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV21::HighActivity).unwrap(),
            "\"high_activity\""
        );
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV21::ComplianceDrift).unwrap(),
            "\"compliance_drift\""
        );
        assert_eq!(
            serde_json::to_string(&AnomalyTypeV21::GeographicAnomaly).unwrap(),
            "\"geographic_anomaly\""
        );
    }

    #[test]
    fn test_risk_level_v21_serialization() {
        assert_eq!(
            serde_json::to_string(&RiskLevelV21::Critical).unwrap(),
            "\"critical\""
        );
        assert_eq!(
            serde_json::to_string(&RiskLevelV21::Low).unwrap(),
            "\"low\""
        );
    }
}
