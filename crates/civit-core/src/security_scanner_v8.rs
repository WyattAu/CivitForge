...
pub struct SecurityScanRuleV8 {
    pub id: String,
    pub name: String,
    pub description: String,
    pub rule_type: RuleTypeV8,
    pub severity: SeverityV8,
    pub pattern: String,
    pub enabled: bool,
    pub version: u32,
    ...
    pub compliance_mapping: ComplianceMappingV8,
    pub created_at: DateTime<Utc>,
    pub updated_at: DateTime<Utc>,
}

impl SecurityScanRuleV8 {
    pub fn new(name: String, description: String, rule_type: RuleTypeV8, pattern: String) -> Self {
        let now = Utc::now();
        Self {
            ...
            compliance_mapping: ComplianceMappingV8::default(),
            created_at: now,
            updated_at: now,
        }
    }

    pub fn with_severity(mut self, severity: SeverityV8) -> Self {
    ...
    pub fn with_compliance_mapping(mut self, mapping: ComplianceMappingV8) -> Self {
...
pub struct ComplianceMappingV8 {
    pub mappings: Vec<ComplianceMappingEntryV8>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceMappingEntryV8 {
    pub framework_id: String,
    pub framework_name: String,
    pub requirement_id: String,
    pub mapping_type: ComplianceMappingTypeV8,
    pub confidence: f64,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ComplianceMappingTypeV8 {
    Direct,
    Partial,
    Advisory,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScanFixV8 {
    pub id: String,
    pub scan_id: String,
    pub rule_id: String,
    pub file_path: String,
    pub line_number: Option<u32>,
    pub fix_type: FixTypeV8,
    pub fix_content: String,
    pub applied: bool,
    pub test_result: Option<FixTestResultV8>,
    pub created_at: DateTime<Utc>,
}
...
pub struct FixAnalyticsV8 {
    ...
    pub compliance_impact: ComplianceImpactV8,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ComplianceImpactV8 {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RuleVersionHistoryV8 {
    pub rule_id: String,
    pub versions: Vec<RuleVersionEntryV8>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RuleVersionEntryV8 {
    pub version: u32,
    pub pattern: String,
    pub changed_by: Option<String>,
    pub changed_at: DateTime<Utc>,
    pub change_notes: Option<String>,
    pub compliance_mappings_snapshot: ComplianceMappingV8,
}

#[derive(Debug, Clone)]
pub struct SecurityScanRuleManagerV8 {
    rules: Vec<SecurityScanRuleV8>,
    fixes: Vec<ScanFixV8>,
    version_history: HashMap<String, Vec<RuleVersionEntryV8>>,
}

impl Default for SecurityScanRuleManagerV8 {
    fn default() -> Self {
        Self::new()
    }
}

impl SecurityScanRuleManagerV8 {
    pub fn new() -> Self {
        Self {
            rules: Vec::new(),
            fixes: Vec::new(),
    ...
    pub fn add_rule(&mut self, rule: SecurityScanRuleV8) {
        let entry = RuleVersionEntryV8 {
    ...
    pub fn get_rule(&self, rule_id: &str) -> Option<&SecurityScanRuleV8> {
        self.rules.iter().find(|r| r.id == rule_id)
    }

    pub fn get_rule_mut(&mut self, rule_id: &str) -> Option<&mut SecurityScanRuleV8> {
        self.rules.iter_mut().find(|r| r.id == rule_id)
    }

    pub fn list_rules(&self) -> &[SecurityScanRuleV8] {
        &self.rules
    }

    pub fn list_enabled_rules(&self) -> Vec<&SecurityScanRuleV8> {
        self.rules.iter().filter(|r| r.enabled).collect()
    }

    pub fn list_auto_fix_rules(&self) -> Vec<&SecurityScanRuleV8> {
        self.rules.iter().filter(|r| r.auto_fix && r.enabled).collect()
    }

    pub fn list_rules_by_severity(&self, severity: SeverityV8) -> Vec<&SecurityScanRuleV8> {
    ...
    pub fn get_version_history(&self, rule_id: &str) -> Option<&Vec<RuleVersionEntryV8>> {
        self.version_history.get(rule_id)
    }

    pub fn test_rule(&self, rule: &SecurityScanRuleV8, input: &str) -> RuleTestResultV8 {
        let start = std::time::Instant::now();
        let mut matched_lines = Vec::new();

        for (idx, line) in input.lines().enumerate() {
            let matched = match rule.rule_type {
                RuleTypeV8::Regex => regex::Regex::new(&rule.pattern)
                    .map(|re| re.is_match(line))
                    .unwrap_or(false),
                RuleTypeV8::Keyword => line.contains(&rule.pattern),
                RuleTypeV8::Pattern => line.to_lowercase().contains(&rule.pattern.to_lowercase()),
                RuleTypeV8::Ast | RuleTypeV8::Custom => false,
            };
            if matched {
    ...
    pub fn record_fix(&mut self, fix: ScanFixV8) {
        self.fixes.push(fix);
    }
    ...
    pub fn test_fix(&mut self, fix_id: &str, test_input: &str) -> Result<FixTestResultV8, String> {
        let fix = self
            .fixes
            .iter_mut()
            .find(|f| f.id == fix_id)
    ...
    pub fn get_fixes_for_scan(&self, scan_id: &str) -> Vec<&ScanFixV8> {
        self.fixes.iter().filter(|f| f.scan_id == scan_id).collect()
    }

    pub fn get_pending_fixes(&self) -> Vec<&ScanFixV8> {
        self.fixes.iter().filter(|f| !f.applied).collect()
    }

    pub fn compute_fix_analytics(&self) -> FixAnalyticsV8 {
        ...
        let compliance_impact = ComplianceImpactV8 {
    ...
    pub fn generate_report(&self) -> String {
        let mut report = String::from("=== Security Scan V8 Report ===\n\n");
...
pub fn create_default_scan_rules_v8() -> Vec<SecurityScanRuleV8> {
    vec![
        SecurityScanRuleV8::new(
            "Hardcoded Secret".into(),
            "Detects hardcoded secrets and API keys".into(),
            RuleTypeV8::Regex,
            r#"(?i)(api[_-]?key|secret|password|token)\s*[:=]\s*['"][^'"]+['"]"#.into(),
        )
        .with_severity(SeverityV8::Critical)
        .with_compliance_mapping(ComplianceMappingV8 {
            mappings: vec![ComplianceMappingEntryV8 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
                requirement_id: "CC6.1".into(),
                mapping_type: ComplianceMappingTypeV8::Direct,
                confidence: 0.95,
            }],
        }),
        SecurityScanRuleV8::new(
...
mod tests {
    ...
    fn sample_rule(id: &str, severity: SeverityV8, auto_fix: bool) -> SecurityScanRuleV8 {
        SecurityScanRuleV8 {
            id: id.to_string(),
            name: format!("Rule {id}"),
            description: format!("Description for rule {id}"),
            rule_type: RuleTypeV8::Pattern,
            severity,
    ...
    fn sample_fix(id: &str, scan_id: &str, rule_id: &str, applied: bool) -> ScanFixV8 {
        ScanFixV8 {
            id: id.to_string(),
            ...
            line_number: Some(42),
            fix_type: FixTypeV8::PatternReplace,
    ...
    #[test]
    fn test_add_rule() {
        let mut manager = SecurityScanRuleManagerV8::new();
        manager.add_rule(sample_rule("r1", SeverityV8::High, false));
    ...
    fn test_get_rules_by_severity() {
        let mut manager = SecurityScanRuleManagerV8::new();
        manager.add_rule(sample_rule("r1", SeverityV8::High, false));
        manager.add_rule(sample_rule("r2", SeverityV8::Critical, false));
        manager.add_rule(sample_rule("r3", SeverityV8::High, false));
        let high = manager.list_rules_by_severity(SeverityV8::High);
    ...
    fn test_get_auto_fix_rules() {
        let mut manager = SecurityScanRuleManagerV8::new();
        manager.add_rule(sample_rule("r1", SeverityV8::High, true));
        manager.add_rule(sample_rule("r2", SeverityV8::Critical, false));
    ...
    fn test_generate_report() {
        ...
        assert!(report.contains("Security Scan V8 Report"));
    ...
    fn test_compliance_mapping() {
        let rule = SecurityScanRuleV8::new(
            "Test".into(),
            "Desc".into(),
            RuleTypeV8::Regex,
            "pattern".into(),
    ...
    fn test_severity_risk_weight() {
        assert_eq!(SeverityV8::Critical.risk_weight(), 4);
        assert_eq!(SeverityV8::High.risk_weight(), 3);
        assert_eq!(SeverityV8::Medium.risk_weight(), 2);
        assert_eq!(SeverityV8::Low.risk_weight(), 1);
        assert_eq!(SeverityV8::Informational.risk_weight(), 0);
    }

    #[test]
    fn test_empty_manager() {
        let manager = SecurityScanRuleManagerV8::new();
    ...
    fn test_fix_analytics_with_compliance() {
        let mut manager = SecurityScanRuleManagerV8::new();
        let mut rule = sample_rule("r1", SeverityV8::High, true);
        rule.compliance_mapping = ComplianceMappingV8 {
            mappings: vec![ComplianceMappingEntryV8 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
                requirement_id: "CC6.1".into(),
    ...
    fn test_default_rules_have_compliance_mapping() {
        let rules = create_default_scan_rules_v8();
