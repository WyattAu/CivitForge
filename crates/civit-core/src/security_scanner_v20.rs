...
pub struct SecurityScanRuleV20 {
    pub id: String,
    pub name: String,
    pub description: String,
    pub rule_type: RuleTypeV20,
    pub severity: SeverityV20,
    pub pattern: String,
    pub enabled: bool,
    pub version: u32,
    ...
    pub compliance_mapping: ComplianceMappingV20,
    pub created_at: DateTime<Utc>,
    pub updated_at: DateTime<Utc>,
}

impl SecurityScanRuleV20 {
    pub fn new(name: String, description: String, rule_type: RuleTypeV20, pattern: String) -> Self {
        let now = Utc::now();
        Self {
            ...
            compliance_mapping: ComplianceMappingV20::default(),
            created_at: now,
            updated_at: now,
        }
    }

    pub fn with_severity(mut self, severity: SeverityV20) -> Self {
    ...
    pub fn with_compliance_mapping(mut self, mapping: ComplianceMappingV20) -> Self {
...
pub struct ComplianceMappingV20 {
    pub mappings: Vec<ComplianceMappingEntryV20>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceMappingEntryV20 {
    pub framework_id: String,
    pub framework_name: String,
    pub requirement_id: String,
    pub mapping_type: ComplianceMappingTypeV20,
    pub confidence: f64,
...
#[serde(rename_all = "snake_case")]
pub enum ComplianceMappingTypeV20 {
    Direct,
    Partial,
    Advisory,
    Inherited,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScanFixV20 {
    pub id: String,
    pub scan_id: String,
    pub rule_id: String,
    pub file_path: String,
    pub line_number: Option<u32>,
    pub fix_type: FixTypeV20,
    pub fix_content: String,
    pub applied: bool,
    pub test_result: Option<FixTestResultV20>,
...
pub struct FixAnalyticsV20 {
    ...
    pub compliance_impact: ComplianceImpactV20,
    pub rollback_count: u64,
    pub avg_apply_time_ms: f64,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ComplianceImpactV20 {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RuleVersionHistoryV20 {
    pub rule_id: String,
    pub versions: Vec<RuleVersionEntryV20>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RuleVersionEntryV20 {
    pub version: u32,
    pub pattern: String,
    pub changed_by: Option<String>,
    pub changed_at: DateTime<Utc>,
    pub change_notes: Option<String>,
    pub compliance_mappings_snapshot: ComplianceMappingV20,
    pub breaking_change: bool,
}

#[derive(Debug, Clone)]
pub struct SecurityScanRuleManagerV20 {
    rules: Vec<SecurityScanRuleV20>,
    fixes: Vec<ScanFixV20>,
    version_history: HashMap<String, Vec<RuleVersionEntryV20>>,
}

impl Default for SecurityScanRuleManagerV20 {
    fn default() -> Self {
        Self::new()
    }
}

impl SecurityScanRuleManagerV20 {
    pub fn new() -> Self {
        Self {
            rules: Vec::new(),
            fixes: Vec::new(),
    ...
    pub fn add_rule(&mut self, rule: SecurityScanRuleV20) {
        let entry = RuleVersionEntryV20 {
    ...
    pub fn get_rule(&self, rule_id: &str) -> Option<&SecurityScanRuleV20> {
        self.rules.iter().find(|r| r.id == rule_id)
    }

    pub fn get_rule_mut(&mut self, rule_id: &str) -> Option<&mut SecurityScanRuleV20> {
        self.rules.iter_mut().find(|r| r.id == rule_id)
    }

    pub fn list_rules(&self) -> &[SecurityScanRuleV20] {
        &self.rules
    }

    pub fn list_enabled_rules(&self) -> Vec<&SecurityScanRuleV20> {
        self.rules.iter().filter(|r| r.enabled).collect()
    }

    pub fn list_auto_fix_rules(&self) -> Vec<&SecurityScanRuleV20> {
        self.rules.iter().filter(|r| r.auto_fix && r.enabled).collect()
    }

    pub fn list_rules_by_severity(&self, severity: SeverityV20) -> Vec<&SecurityScanRuleV20> {
        self.rules.iter().filter(|r| r.severity == severity).collect()
    }

    pub fn list_rules_with_compliance(&self) -> Vec<&SecurityScanRuleV20> {
        self.rules
            .iter()
    ...
    pub fn list_auto_verifiable_rules(&self) -> Vec<&SecurityScanRuleV20> {
        self.rules
            .iter()
            .filter(|r| {
                r.enabled
    ...
    pub fn get_version_history(&self, rule_id: &str) -> Option<&Vec<RuleVersionEntryV20>> {
        self.version_history.get(rule_id)
    }

    pub fn test_rule(&self, rule: &SecurityScanRuleV20, input: &str) -> RuleTestResultV20 {
        let start = std::time::Instant::now();
        let mut matched_lines = Vec::new();

        for (idx, line) in input.lines().enumerate() {
            let matched = match rule.rule_type {
                RuleTypeV20::Regex => regex::Regex::new(&rule.pattern)
                    .map(|re| re.is_match(line))
                    .unwrap_or(false),
                RuleTypeV20::Keyword => line.contains(&rule.pattern),
                RuleTypeV20::Pattern => line.to_lowercase().contains(&rule.pattern.to_lowercase()),
                RuleTypeV20::Ast | RuleTypeV20::Custom | RuleTypeV20::Composite => false,
            };
            if matched {
    ...
    pub fn record_fix(&mut self, fix: ScanFixV20) {
        self.fixes.push(fix);
    }
    ...
    pub fn test_fix(&mut self, fix_id: &str, test_input: &str) -> Result<FixTestResultV20, String> {
        let fix = self
            .fixes
            .iter_mut()
            .find(|f| f.id == fix_id)
    ...
    pub fn get_fixes_for_scan(&self, scan_id: &str) -> Vec<&ScanFixV20> {
        self.fixes.iter().filter(|f| f.scan_id == scan_id).collect()
    }

    pub fn get_pending_fixes(&self) -> Vec<&ScanFixV20> {
        self.fixes.iter().filter(|f| !f.applied).collect()
    }

    pub fn get_rollbackable_fixes(&self) -> Vec<&ScanFixV20> {
        self.fixes
            .iter()
    ...
    pub fn compute_fix_analytics(&self) -> FixAnalyticsV20 {
        ...
        for rule in &self.rules {
            ...
            if self.fixes.iter().any(|f| f.rule_id == rule.id) {
                *fixes_by_severity
                ...
                if !rule.compliance_mapping.mappings.is_empty() {
        ...
        let compliance_impact = ComplianceImpactV20 {
    ...
    pub fn generate_report(&self) -> String {
        let mut report = String::from("=== Security Scan V20 Report ===\n\n");
        ...
        for severity in &[
            SeverityV20::Critical,
            SeverityV20::High,
            SeverityV20::Medium,
            SeverityV20::Low,
            SeverityV20::Informational,
        ] {
...
pub fn create_default_scan_rules_v19() -> Vec<SecurityScanRuleV20> {
    vec![
        SecurityScanRuleV20::new(
            "Hardcoded Secret".into(),
            "Detects hardcoded secrets and API keys".into(),
            RuleTypeV20::Regex,
            r#"(?i)(api[_-]?key|secret|password|token)\s*[:=]\s*['"][^'"]+['"]"#.into(),
        )
        .with_severity(SeverityV20::Critical)
        .with_compliance_mapping(ComplianceMappingV20 {
            mappings: vec![ComplianceMappingEntryV20 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
                requirement_id: "CC6.1".into(),
                mapping_type: ComplianceMappingTypeV20::Direct,
                confidence: 0.95,
        ...
        .with_severity(SeverityV20::High)
        .with_compliance_mapping(ComplianceMappingV20 {
            mappings: vec![ComplianceMappingEntryV20 {
...
mod tests {
    ...
    fn sample_rule(id: &str, severity: SeverityV20, auto_fix: bool) -> SecurityScanRuleV20 {
        SecurityScanRuleV20 {
            id: id.to_string(),
            name: format!("Rule {id}"),
            description: format!("Description for rule {id}"),
            rule_type: RuleTypeV20::Pattern,
            severity,
    ...
    fn sample_fix(id: &str, scan_id: &str, rule_id: &str, applied: bool) -> ScanFixV20 {
        ScanFixV20 {
            id: id.to_string(),
            ...
            line_number: Some(42),
            fix_type: FixTypeV20::PatternReplace,
    ...
    #[test]
    fn test_add_rule() {
        let mut manager = SecurityScanRuleManagerV20::new();
        manager.add_rule(sample_rule("r1", SeverityV20::High, false));
    ...
    fn test_get_rules_by_severity() {
        let mut manager = SecurityScanRuleManagerV20::new();
        manager.add_rule(sample_rule("r1", SeverityV20::High, false));
        manager.add_rule(sample_rule("r2", SeverityV20::Critical, false));
        manager.add_rule(sample_rule("r3", SeverityV20::High, false));
        let high = manager.list_rules_by_severity(SeverityV20::High);
    ...
    fn test_get_auto_fix_rules() {
        let mut manager = SecurityScanRuleManagerV20::new();
        manager.add_rule(sample_rule("r1", SeverityV20::High, true));
        manager.add_rule(sample_rule("r2", SeverityV20::Critical, false));
    ...
    fn test_generate_report() {
        ...
        assert!(report.contains("Security Scan V20 Report"));
    ...
    fn test_compliance_mapping() {
        let rule = SecurityScanRuleV20::new(
            "Test".into(),
            "Desc".into(),
            RuleTypeV20::Regex,
            "pattern".into(),
    ...
    fn test_severity_risk_weight() {
        assert_eq!(SeverityV20::Critical.risk_weight(), 5);
        assert_eq!(SeverityV20::High.risk_weight(), 4);
        assert_eq!(SeverityV20::Medium.risk_weight(), 2);
        assert_eq!(SeverityV20::Low.risk_weight(), 1);
        assert_eq!(SeverityV20::Informational.risk_weight(), 0);
    }

    #[test]
    fn test_severity_display_name() {
        assert_eq!(SeverityV20::Critical.display_name(), "Critical");
        assert_eq!(SeverityV20::High.display_name(), "High");
    }

    #[test]
    fn test_empty_manager() {
        let manager = SecurityScanRuleManagerV20::new();
    ...
    fn test_fix_analytics_with_compliance() {
        let mut manager = SecurityScanRuleManagerV20::new();
        let mut rule = sample_rule("r1", SeverityV20::High, true);
        rule.compliance_mapping = ComplianceMappingV20 {
            mappings: vec![ComplianceMappingEntryV20 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
                requirement_id: "CC6.1".into(),
    ...
    fn test_default_rules_have_compliance_mapping() {
        let rules = create_default_scan_rules_v19();
    ...
    fn test_list_rules_with_compliance() {
        let mut manager = SecurityScanRuleManagerV20::new();
        let mut rule = sample_rule("r1", SeverityV20::High, false);
        rule.compliance_mapping = ComplianceMappingV20 {
            mappings: vec![ComplianceMappingEntryV20 {
                framework_id: "soc2".into(),
                framework_name: "SOC 2".into(),
        ...
        manager.add_rule(rule);
        manager.add_rule(sample_rule("r2", SeverityV20::Low, false));
    ...
    fn test_rule_risk_score() {
        let rule = sample_rule("r1", SeverityV20::Critical, false);
